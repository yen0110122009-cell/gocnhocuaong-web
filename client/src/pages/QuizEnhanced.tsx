...
export default function QuizEnhanced({ profile, config, onProfile }: Props) {
  ...
  const finishPaper = () => {
    ...
    const existing = profile.paperQuizSessions ?? [];
    const activeSessions = [completed, ...existing.filter((session) => !session.deletedAt && session.id !== completed.id)].slice(0, 50);
    const deletedSessions = existing.filter((session) => session.deletedAt && session.id !== completed.id);
    const sessions = [...activeSessions, ...deletedSessions];
...
function ResultView({ active, result, answers, mode, attemptId, profile, onProfile, deepOpen, setDeepOpen, onReset }: { active: Quiz; result: number; answers: Record<string, string>; mode: QuizMode; attemptId: string; profile: ProfileState; onProfile: (profile: ProfileState, message?: string) => void; deepOpen: Record<string, boolean>; setDeepOpen: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; onReset: () => void }) {
  const wrongQuestions = active.questions.filter((question) => {
    const answer = normalizeQuizAnswer(answers[question.id] ?? "");
    const expected = normalizeQuizAnswer(question.answer);
    const numericAnswer = answer.match(/^\d+(?:[.,]\d+)?$/)?.[0];
    const expectedNumbers: string[] = expected.match(/\d+(?:[.,]\d+)?/g) ?? [];
    const correct = answer === expected || Boolean(numericAnswer && expectedNumbers.includes(numericAnswer));
    return !correct;
  });
...
function PaperSessionArchive({ profile, onProfile }: { profile: ProfileState; onProfile: (profile: ProfileState, message?: string) => void }) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [draftNotes, setDraftNotes] = useState("");
  const sessions = profile.paperQuizSessions ?? [];
  const activeSessions = sessions.filter((session) => !session.deletedAt);
  const deletedSessions = sessions.filter((session) => session.deletedAt);
  const replace = (id: string, update: Partial<PaperQuizSession>, message: string) => onProfile({ ...profile, paperQuizSessions: sessions.map((session) => session.id === id ? { ...session, ...update } : session) }, message);
  if (!sessions.length) return null;
  return <section className="panel mt-6 p-5" aria-labelledby="paper-session-archive-title"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-amber-700">Dữ liệu riêng</p><h2 id="paper-session-archive-title" className="font-display text-xl font-bold">Kho phiên Làm đề giấy</h2><p className="mt-1 text-sm text-slate-500">Mỗi phiên có thể sửa ghi chú, đưa vào thùng rác, khôi phục hoặc xóa vĩnh viễn.</p></div><span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800 dark:bg-amber-400/10 dark:text-amber-100">{activeSessions.length} đang lưu</span></div>{activeSessions.length > 0 && <div className="mt-4 space-y-3">{activeSessions.map((session) => <article className="rounded-xl border border-slate-200 p-4 dark:border-white/10" key={session.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><b>{session.title}</b><p className="mt-1 text-xs text-slate-500">{session.subject} · {session.questionCount} câu · {new Date(session.startedAt).toLocaleString("vi-VN")} · {session.status === "completed" ? "Đã hoàn thành" : session.status === "abandoned" ? "Đã bỏ dở" : "Đang lưu"}</p>{session.goal && <p className="mt-2 text-xs font-semibold text-amber-800 dark:text-amber-200">Mục tiêu: {session.goal}</p>}</div><div className="flex flex-wrap gap-2"><button aria-label={`Sửa ghi chú phiên ${session.title}`} className="secondary-button !px-3 !py-2" onClick={() => { setEditingId(session.id); setDraftNotes(session.notes ?? ""); }}><Pencil className="h-3.5 w-3.5" />Sửa</button><button aria-label={`Đưa phiên ${session.title} vào thùng rác`} className="secondary-button !px-3 !py-2 text-rose-700" onClick={() => replace(session.id, { deletedAt: new Date().toISOString() }, "Đã đưa phiên đề giấy vào Thùng rác.")}><Trash2 className="h-3.5 w-3.5" />Xóa</button></div></div>{editingId === session.id ? <div className="mt-3"><textarea aria-label={`Ghi chú phiên ${session.title}`} value={draftNotes} onChange={(event) => setDraftNotes(event.target.value)} rows={3} className="textarea" placeholder="Ghi chú sau phiên…" /><div className="mt-2 flex gap-2"><button className="primary-button !px-3 !py-2" onClick={() => { replace(session.id, { notes: draftNotes }, "Đã cập nhật ghi chú phiên đề giấy."); setEditingId(null); }}><CheckCircle2 className="h-3.5 w-3.5" />Lưu</button><button className="secondary-button !px-3 !py-2" onClick={() => setEditingId(null)}>Hủy</button></div></div> : session.notes ? <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">{session.notes}</p> : <p className="mt-3 text-xs text-slate-500">Chưa có ghi chú.</p>}</article>)}</div>}{deletedSessions.length > 0 && <details className="mt-5 rounded-xl border border-dashed border-slate-300 p-4 dark:border-white/15"><summary className="cursor-pointer text-sm font-bold">Thùng rác phiên đề giấy · {deletedSessions.length}</summary><div className="mt-3 space-y-2">{deletedSessions.map((session) => <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-slate-50 p-3 text-sm dark:bg-white/5" key={session.id}><span>{session.title} · đã xóa {session.deletedAt ? new Date(session.deletedAt).toLocaleDateString("vi-VN") : ""}</span><div className="flex gap-2"><button aria-label={`Khôi phục phiên ${session.title}`} className="secondary-button !px-3 !py-2" onClick={() => replace(session.id, { deletedAt: undefined }, "Đã khôi phục phiên đề giấy.")}><Undo2 className="h-3.5 w-3.5" />Khôi phục</button><button aria-label={`Xóa vĩnh viễn phiên ${session.title}`} className="secondary-button !px-3 !py-2 text-rose-700" onClick={() => onProfile({ ...profile, paperQuizSessions: sessions.filter((item) => item.id !== session.id) }, "Đã xóa vĩnh viễn phiên đề giấy.")}><Trash2 className="h-3.5 w-3.5" />Xóa vĩnh viễn</button></div></div>)}</div></details>}</section>;
}
