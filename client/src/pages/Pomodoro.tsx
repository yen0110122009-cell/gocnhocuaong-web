...
import { applyStudyActivityRewards, computedAchievements, type AppConfig, type AudioActionLog, type PersonalAudioAsset, type PersonalStudyPreset, type PomodoroSession, type PersonalPomodoroAmbientPreset, type ProfileState } from "../../../shared/study";
...
export default function Pomodoro({ profile, config, onProfile, onView, isVisible = true }: Props) {
  ...
  function restorePersistedSession() {
    ...
    setBackgroundSound(saved.backgroundSound);
    setBackgroundVolume(saved.backgroundVolume);
    setLayerVolumes(saved.layerVolumes);
    setAlertVolume(saved.alertVolume);
    setPomodoroAmbientMix(saved.pomodoroAmbientMix);
  ...
  async function startBackground(allowWhenJustEnabled = false) {
    ...
    try {
      ...
      if (backgroundSound === "Bình minh & Bão nhẹ") {
        const morning = new Audio(DEFAULT_AMBIENT_MORNING_URL);
        const storm = new Audio(DEFAULT_AMBIENT_STORM_URL);
        morning.loop = true; storm.loop = true;
        const totalMix = Math.max(1, audioAmbientMixSnapshot.morning + audioAmbientMixSnapshot.storm);
        morning.volume = Math.min(1, backgroundVolume / 100 * audioAmbientMixSnapshot.morning / totalMix);
        storm.volume = Math.min(1, backgroundVolume / 100 * audioAmbientMixSnapshot.storm / totalMix);
        await Promise.all([morning.play(), storm.play()]);
        if (generation !== backgroundGenerationRef.current) { morning.pause(); storm.pause(); return false; }
        backgroundStopRef.current = () => { fadeOutAudio(morning); fadeOutAudio(storm); };
  ...
  function reset(record = false) {
    if (record && running && mode === "focus" && sessionStartedAt) {
      ...
      const abandoned: PomodoroSession = { id: crypto.randomUUID(), startedAt: sessionStartedAt, endedAt: new Date().toISOString(), durationMinutes: Math.floor(elapsed / 60), subject, topic, sessionNumber: completedToday + 1, totalSessions, mode: "focus", status: "abandoned", audioPresetId: audioPresetSnapshot?.id, audioPresetName: audioPresetSnapshot?.name ?? selectedSoundscape.label, audioAmbientMix: audioAmbientMixSnapshot };
  ...
  function endEarly() {
    ...
    const item: PomodoroSession = { id: crypto.randomUUID(), startedAt: sessionStartedAt, endedAt: now, durationMinutes: Math.floor(elapsedSeconds / 60), subject, topic, sessionNumber: cyclePosition + 1, totalSessions, mode: "focus", status: "abandoned", audioPresetId: audioPresetSnapshot?.id, audioPresetName: audioPresetSnapshot?.name ?? selectedSoundscape.label, audioAmbientMix: audioAmbientMixSnapshot };
  ...
  const personalAmbientPresets = profile.personalPomodoroAmbientPresets ?? [];
  function applyPersonalAmbientPreset(preset: PersonalPomodoroAmbientPreset) {
    const next = { morning: clamp(preset.morning, 0, 100), storm: clamp(preset.storm, 0, 100) };
    setPomodoroAmbientMix(next);
    setAppliedAmbientPresetId(preset.id);
    if (appliedAmbientPresetTimerRef.current !== null) window.clearTimeout(appliedAmbientPresetTimerRef.current);
    appliedAmbientPresetTimerRef.current = window.setTimeout(() => {
      setAppliedAmbientPresetId(null);
      appliedAmbientPresetTimerRef.current = null;
    }, 900);
    toast.success(`Đã áp dụng preset âm lượng “${preset.name}”.`);
    onProfile({ ...profile, audioMixer: profile.audioMixer ? { ...profile.audioMixer, pomodoroAmbientMix: next } : undefined }, `Đã áp dụng preset âm lượng “${preset.name}”.`);
  }
  function savePersonalAmbientPreset() {
    const name = ambientPresetName.trim().slice(0, 60);
    if (!name) { toast.error("Hãy đặt tên cho preset âm lượng."); return; }
    const now = new Date().toISOString();
    const nextPreset: PersonalPomodoroAmbientPreset = { id: editingAmbientPresetId ?? `ambient-mix-${Date.now()}`, name, morning: clamp(pomodoroAmbientMix.morning, 0, 100), storm: clamp(pomodoroAmbientMix.storm, 0, 100), createdAt: editingAmbientPresetId ? (personalAmbientPresets.find((item) => item.id === editingAmbientPresetId)?.createdAt ?? now) : now, updatedAt: now };
    const next = editingAmbientPresetId ? personalAmbientPresets.map((item) => item.id === editingAmbientPresetId ? nextPreset : item) : [nextPreset, ...personalAmbientPresets];
    onProfile({ ...profile, personalPomodoroAmbientPresets: next }, editingAmbientPresetId ? `Đã cập nhật preset “${name}”.` : `Đã lưu preset “${name}”.`);
    setAmbientPresetName(""); setEditingAmbientPresetId(null);
  }
  function editPersonalAmbientPreset(preset: PersonalPomodoroAmbientPreset) { setAmbientPresetName(preset.name); setPomodoroAmbientMix({ morning: preset.morning, storm: preset.storm }); setEditingAmbientPresetId(preset.id); }
  function deletePersonalAmbientPreset(id: string) {
    const preset = personalAmbientPresets.find((item) => item.id === id);
    if (!preset || !window.confirm(`Xóa preset “${preset.name}” khỏi thư viện cá nhân?`)) return;
    onProfile({ ...profile, personalPomodoroAmbientPresets: personalAmbientPresets.filter((item) => item.id !== id) }, `Đã xóa preset “${preset.name}”.`);
    if (editingAmbientPresetId === id) { setEditingAmbientPresetId(null); setAmbientPresetName(""); }
  }
  ...
  if (!isVisible) {
    ...
    return <aside className="fixed bottom-4 right-4 z-[80] w-[min(92vw,24rem)] rounded-2xl border border-emerald-300/40 bg-slate-950/95 p-4 text-white shadow-2xl backdrop-blur" aria-label="Mini player âm thanh và Pomodoro"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-emerald-300">Pomodoro đang chạy</p><p className="mt-1 font-mono text-3xl font-black" aria-live="polite">{display}</p><p className="text-xs text-slate-300">{modeLabels[mode]} · {backgroundActive ? "Âm nền đang phát" : "Đang giữ phiên"}</p><span className={`mt-2 inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-[10px] font-black transition-all duration-300 ${audioUnlocked ? "bg-emerald-400/15 text-emerald-300" : "bg-amber-400/15 text-amber-200"}`} role="status" aria-label={audioUnlocked ? "Âm thanh đã mở khóa trên thiết bị" : "Âm thanh chưa mở khóa trên thiết bị"}>{audioUnlocked ? "✓ Âm thanh đã mở khóa" : "◌ Chạm để mở khóa âm thanh"}</span></div><button type="button" className="rounded-lg px-2 py-1 text-xs font-bold text-emerald-200 hover:bg-white/10" onClick={() => onView("pomodoro")}>Mở</button></div><div className="mt-3 flex gap-2"><button type="button" className="primary-button flex-1" onClick={handleMainAction}>{running ? "Tạm dừng" : "Bắt đầu"}</button><button type="button" className="secondary-button" onClick={() => setMiniPlayerPinned(false)}>Bỏ ghim</button></div></aside>;
  }
