...
import { applyStudyActivityRewards, computedAchievements, type AppConfig, type AudioActionLog, type PersonalAudioAsset, type PersonalStudyPreset, type PomodoroSession, type ProfileState, normalizeProfile } from "../../../shared/study";
...
export default function Pomodoro({ profile, config, onProfile, onView, isVisible = true }: Props) {
  ...
  const [themeAmbientEnabled, setThemeAmbientEnabled] = useState(false);
  const themeAmbientAsset = PROVIDED_THEME_AMBIENT_ASSETS[profile.defaultAmbientScene as keyof typeof PROVIDED_THEME_AMBIENT_ASSETS];
  const themeAmbientVolume = profile.audioMixer?.pomodoroBackground ?? 40;
  ...
  function stopThemeAmbient() {
    const audio = themeAmbientRef.current;
    if (audio) { audio.pause(); audio.currentTime = 0; }
    themeAmbientRef.current = null;
  }
  useEffect(() => {
    stopThemeAmbient();
    if (!themeAmbientEnabled || !sound || !themeAmbientAsset || themeAmbientVolume <= 0) return;
    const audio = new Audio(themeAmbientAsset.url);
    audio.loop = true;
    audio.volume = clamp((themeAmbientAsset.source === "provided" ? 0.45 : 0.35) * themeAmbientVolume / 100, 0, 1);
    themeAmbientRef.current = audio;
    void audio.play().catch(() => toast.error("Không thể phát âm nền theme. Hãy nhấn Bật âm thanh rồi thử lại."));
    return () => stopThemeAmbient();
  }, [themeAmbientEnabled, sound, themeAmbientAsset, themeAmbientVolume]);
