...
export default function Pomodoro({ profile, config, onProfile, onView, isVisible = true }: Props) {
  ...
  if (!isVisible) {
    ...
    return <aside className="fixed bottom-4 right-4 z-[80] w-[min(92vw,24rem)] rounded-2xl border border-emerald-300/40 bg-slate-950/95 p-4 text-white shadow-2xl backdrop-blur" aria-label="Mini player âm thanh và Pomodoro"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-emerald-300">Pomodoro đang chạy</p><p className="mt-1 font-mono text-3xl font-black" aria-live="polite">{display}</p><p className="text-xs text-slate-300">{modeLabels[mode]} · 🔔 Chỉ âm báo trạng thái</p><span className={`mt-2 inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-[10px] font-black transition-all duration-300 ${audioUnlocked ? "bg-emerald-400/15 text-emerald-300" : "bg-amber-400/15 text-amber-200"}`} role="status" aria-label={audioUnlocked ? "Âm thanh đã mở khóa trên thiết bị" : "Âm thanh chưa mở khóa trên thiết bị"}>{audioUnlocked ? "✓ Âm thanh đã mở khóa" : "◌ Chạm để mở khóa âm thanh"}</span></div><button type="button" className="rounded-lg px-2 py-1 text-xs font-bold text-emerald-200 hover:bg-white/10" onClick={() => onView("pomodoro")}>Mở</button></div><div className="mt-3 flex gap-2"><button type="button" className="primary-button flex-1" onClick={handleMainAction}>{mainActionLabel}</button><button type="button" className="secondary-button" onClick={() => setMiniPlayerPinned(false)}>Bỏ ghim</button></div></aside>;
  }
