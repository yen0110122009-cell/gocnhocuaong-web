...
type View = "pomodoro" | "flashcards" | "quizzes" | "history";
type Props = { profile: ProfileState; config: AppConfig; accountId: string; onProfile: (profile: ProfileState, message?: string) => void; onView: (view: View) => void; onOpenDetached?: () => void; isVisible?: boolean };
...
const presets = [{ label: "10 / 5", focus: 10, short: 5, long: 15 }, { label: "25 / 5", focus: 25, short: 5, long: 15 }, { label: "45 / 10", focus: 45, short: 10, long: 20 }, { label: "50 / 10", focus: 50, short: 10, long: 20 }];
type PersonalPreset = { id: string; label: string; focus: number; short: number; long: number };
function readPersonalPresets(accountId: string): PersonalPreset[] { try { const raw = window.localStorage.getItem(`pomodoro_personal_presets:${accountId}`); const value = raw ? JSON.parse(raw) : []; return Array.isArray(value) ? value.filter((item): item is PersonalPreset => item && typeof item.id === "string" && typeof item.label === "string" && Number.isFinite(item.focus) && Number.isFinite(item.short) && Number.isFinite(item.long)).slice(0, 12) : []; } catch { return []; } }
function savePersonalPresets(accountId: string, value: PersonalPreset[]) { try { window.localStorage.setItem(`pomodoro_personal_presets:${accountId}`, JSON.stringify(value.slice(0, 12))); } catch { /* localStorage có thể bị chặn */ } }
...
export default function Pomodoro({ profile, config, accountId, onProfile, onView, onOpenDetached, isVisible = true }: Props) {
  ...
  const [pendingTransition, setPendingTransition] = useState<"break" | "focus" | null>(() => pendingTransitionForSavedPomodoro(restored));
  ...
  const [personalPresets, setPersonalPresets] = useState<PersonalPreset[]>(() => readPersonalPresets(accountId));
  const [subjectConfirmationOpen, setSubjectConfirmationOpen] = useState(false);
  const [sessionSummary, setSessionSummary] = useState<{ subject: string; topic: string; durationMinutes: number; sessionNumber: number; totalSessions: number } | null>(null);
  ...
  const [lumiWaterSettings, setLumiWaterSettings] = useState<LumiWaterSettings>(() => readLumiWaterSettings(profile.lumiWaterSettings));
  const [waterSecondsRemaining, setWaterSecondsRemaining] = useState(() => readLumiWaterSettings(profile.lumiWaterSettings).intervalMinutes * 60);
  ...
  useEffect(() => {
    const next = readLumiWaterSettings(profile.lumiWaterSettings);
  ...
  function triggerPomodoroAlert(eventId: PomodoroAlertEventId) {
    ...
    void getFreshAlertContext().then((context) => {
      if (context) playPomodoroAlert(context, eventSettings.soundId, pomodoroAlertSettings.masterVolume, pomodoroAlertSettings.repeatCount, pomodoroAlertSettings.durationMultiplier);
    }).catch(() => undefined);
  }
  ...
  function previewWaterAlert() {
    if (!profile.soundEnabled) { toast.info("Hãy bật âm thanh của trang để nghe thử âm báo."); return; }
    void getFreshAlertContext().then((context) => {
      if (context) playLumiWaterAlert(context, lumiWaterSettings.soundId, lumiWaterSettings.alertVolume, lumiWaterSettings.repeatCount, lumiWaterSettings.durationMultiplier);
    }).catch(() => undefined);
  }

  function triggerLumiWaterAlert() {
    if (!profile.soundEnabled) return;
    void getFreshAlertContext().then((context) => { if (context) playLumiWaterAlert(context, lumiWaterSettings.soundId, lumiWaterSettings.alertVolume, lumiWaterSettings.repeatCount, lumiWaterSettings.durationMultiplier); }).catch(() => undefined);
  }
  ...
  function updateDailyWaterTime(index: number, value: string) {
    const dailyTimes = lumiWaterSettings.dailyTimes ?? [lumiWaterSettings.dailyTime ?? "09:00"];
    const nextValue = value || dailyTimes[index] || "09:00";
    updateDailyWaterTimes(dailyTimes.map((item, itemIndex) => itemIndex === index ? nextValue : item));
  }
  ...
  function dismissSessionSummary() { setSessionSummary(null); }
  function openNextSessionPicker() { if (running) { toast.info("Hãy chờ giờ nghỉ kết thúc trước khi chọn phiên mới."); return; } if (mode !== "focus" || pendingTransition !== "focus") { toast.info("Sau khi giờ nghỉ kết thúc, bấm Bắt đầu phiên để chọn môn và nội dung."); setSessionSummary(null); return; } setSessionSummary(null); setSubjectConfirmationOpen(true); }
  function openSubjectPickerArea() { setSubjectConfirmationOpen(false); setSessionSummary(null); setCompactMode(false); setDeepFocusMode(false); onView("pomodoro"); window.setTimeout(() => { const target = document.getElementById("pomodoro-subject-picker"); target?.scrollIntoView({ behavior: "smooth", block: "center" }); const input = target?.querySelector<HTMLInputElement>("input"); input?.focus(); }, 0); }
  ...
  function restore() {
    ...
    const recovered = recoverRunningSeconds(saved);
    setFocus(saved.focus); setShortBreak(saved.shortBreak); setLongBreak(saved.longBreak); setSeconds(recovered); setMode(saved.mode); setRunning(saved.running && recovered > 0); setAutoAdvance(saved.autoAdvance); setPendingTransition(pendingTransitionForSavedPomodoro(saved)); setSubject(saved.subject); setTopic(saved.topic); setActivity((saved.activity as Activity) ?? "theory"); setNotes(saved.notes ?? ""); setTotalSessions(saved.totalSessions); setGoalCompletedSessions(saved.goalCompletedSessions ?? 0); setSessionStartedAt(saved.sessionStartedAt); setCompactMode(saved.compactMode); setDeepFocusMode(saved.deepFocusMode ?? false); setMiniPlayerPinned(saved.miniPlayerPinned); setMiniPlayerPosition({ x: saved.miniPlayerX, y: saved.miniPlayerY }); completionHandled.current = false;
  ...
  async function getFreshAlertContext() {
    if (alertContextRef.current && alertContextRef.current.state !== "closed") await alertContextRef.current.close().catch(() => undefined);
    alertContextRef.current = null;
    return getAlertContext();
  }
  function saveCurrentPreset() {
    const label = `${focus} / ${shortBreak}`;
    const next = [{ id: `${focus}-${shortBreak}-${longBreak}`, label, focus, short: shortBreak, long: longBreak }, ...personalPresets.filter((preset) => preset.id !== `${focus}-${shortBreak}-${longBreak}`)].slice(0, 12);
    setPersonalPresets(next); savePersonalPresets(accountId, next); toast.success(`Đã lưu nhịp cá nhân ${label}.`);
  }
  function reuseLastSession() {
    const last = recentCompletedSessions[0];
    if (!last) return;
    setSubject(last.subject?.trim() ?? ""); setTopic(last.topic?.trim() ?? ""); setNotes(last.notes ?? "");
    const matchingActivity = activities.find((item) => item.label === last.activity);
    if (matchingActivity) setActivity(matchingActivity.id);
    setMode("focus"); setPendingTransition("focus"); setRunning(false); setSeconds(focus * 60); setSessionSummary(null); setSubjectConfirmationOpen(true);
  }
  function begin() {
    if (running) { setRunning(false); return; }
    if (pendingTransition === "focus" || (mode === "focus" && !sessionStartedAt)) { notifyFocusSessionStart(); setSubjectConfirmationOpen(true); return; }
    startNonFocusSession();
  }
  function startNonFocusSession() {
    ...
    setSeconds(nextSeconds); setRunning(true); completionHandled.current = false; triggerPomodoroAlert(mode === "focus" ? "startFocus" : "startBreak");
  }
  function confirmSubjectAndStart() {
    if (!subject.trim() || subject === "__custom__") { toast.error("Hãy chọn hoặc nhập môn học trước khi bắt đầu."); return; }
    if (!topic.trim()) { toast.error("Hãy nhập nội dung của phiên học trước khi bắt đầu."); return; }
    setSubjectConfirmationOpen(false);
    setSessionSummary(null);
    const nextSeconds = pomodoroStartSeconds({ mode, pendingTransition, seconds, focusMinutes: focus, shortBreakMinutes: shortBreak, longBreakMinutes: longBreak });
    if (pendingTransition === "focus") { setPendingTransition(null); setMode("focus"); setSeconds(nextSeconds); setSessionStartedAt(new Date().toISOString()); }
    else if (goalCompletedSessions >= totalSessions) { setGoalCompletedSessions(0); setSeconds(focus * 60); setSessionStartedAt(new Date().toISOString()); }
    else { setSeconds(nextSeconds); setSessionStartedAt(new Date().toISOString()); }
    setRunning(true); completionHandled.current = false; triggerPomodoroAlert("startFocus");
  }
  ...
  function completeFocus() {
    ...
    if (transition.goalReached) celebrateGoal();
    if (transition.goalReached && !transition.running && transition.mode === "focus") { toast.success(`Chúc mừng Ong đã hoàn thành ${totalSessions} phiên Pomodoro.`); return; }
    if (transition.running) { triggerPomodoroAlert("startBreak"); toast.success(transition.goalReached ? `Đã đủ ${totalSessions} phiên. Pomodoro chuyển sang nghỉ dài.` : "Đã hoàn thành phiên. Pomodoro chuyển sang thời gian nghỉ."); }
    else { toast.success(transition.goalReached ? `Đã đủ ${totalSessions} phiên. Hãy bắt đầu nghỉ dài khi sẵn sàng.` : "Đã hoàn thành phiên. Khi sẵn sàng, bạn có thể bắt đầu nghỉ."); }
  }
  function completeBreak() {
    const startsNewGoalCycle = goalCompletedSessions >= totalSessions;
    if (startsNewGoalCycle) {
      ...
      if (autoAdvance) { setMode("focus"); setPendingTransition("focus"); setRunning(false); setSessionSummary(null); completionHandled.current = false; notifyFocusSessionStart(); setSubjectConfirmationOpen(true); }
      else { setRunning(false); completionHandled.current = false; }
      return;
    }
    if (autoAdvance) { setMode("focus"); setRunning(false); setPendingTransition("focus"); setSessionSummary(null); completionHandled.current = false; notifyFocusSessionStart(); setSubjectConfirmationOpen(true); }
    else { setMode("focus"); setRunning(false); setPendingTransition("focus"); completionHandled.current = false; }
  }
  function choosePreset(value: typeof presets[number]) { if (running && !window.confirm("Đổi nhịp học sẽ dừng phiên hiện tại. Tiếp tục?")) return; setRunning(false); setFocus(value.focus); setShortBreak(value.short); setLongBreak(value.long); setMode("focus"); setGoalCompletedSessions(0); setPendingTransition(null); setSeconds(value.focus * 60); setSessionStartedAt(null); setSubjectConfirmationOpen(false); setSessionSummary(null); }

  const lumiPopups = <>{showLumiDialog ? <div className="modal-backdrop grid place-items-center p-4" role="presentation" onClick={dismissLumiDialog}><section className="lumi-popup-modal relative w-full max-w-md border border-emerald-200 bg-white p-6 pt-16 text-slate-900 shadow-2xl dark:border-emerald-300/25 dark:bg-slate-900 dark:text-slate-100" style={{ left: `${lumiPopupPosition.x}%`, top: `${lumiPopupPosition.y}%`, transform: "translate(-50%, -50%)" }} role="dialog" aria-modal="true" aria-labelledby="lumi-checkin-title" onClick={(event) => event.stopPropagation()}><div className="lumi-popup-timer-card" role="timer" aria-live="off" aria-label={`Lumi đang đếm ngược ${display}`}><span aria-hidden="true">⏱️</span><span>{display}</span></div><div className="lumi-popup-drag-handle touch-none select-none text-center" title="Kéo Lumi để di chuyển popup" onPointerDown={startLumiPopupDrag} onPointerMove={moveLumiPopup} onPointerUp={stopLumiPopupDrag} onPointerCancel={stopLumiPopupDrag}><div className="text-5xl" aria-hidden="true">{lumiDialogResponse?.kaomoji ?? LUMI_WELCOME.kaomoji}</div><p className="mt-3 text-xs font-black uppercase tracking-[.16em] text-emerald-700 dark:text-emerald-300">Lumi hỏi thăm</p><h2 id="lumi-checkin-title" className="mt-1 font-display text-2xl font-black">{lumiDialogResponse ? lumiDialogResponse.group : "Hôm nay Ong cảm thấy thế nào?"}</h2><p className="mt-1 text-xs font-bold text-emerald-700 dark:text-emerald-300">{lumiDialogResponse?.description ?? lumiWelcomeEntry?.description ?? "Lumi sẵn sàng lắng nghe"}</p><p className="mt-2 text-xs font-black text-slate-500 dark:text-slate-300">{sessionProgressLabel} · {labelForMode[mode]}</p><div className="lumi-pomodoro-status-chip mt-3" aria-live="polite">{lumiStatusLabel}</div><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{lumiDialogResponse?.text ?? lumiDialogIntro}</p></div>{!lumiDialogResponse ? <div className="lumi-quick-feelings-grid mt-5">{LUMI_CHECKIN_OPTIONS.map((choice) => <button key={choice.id} type="button" className="secondary-button justify-center text-sm" onClick={() => chooseLumiFeeling(choice.id)}>{choice.emoji} {choice.label}</button>)}</div> : <div className="mt-5 flex gap-2"><button type="button" className="secondary-button flex-1 justify-center" onClick={() => setLumiDialogResponse(null)}>Hỏi lại</button><button type="button" className="primary-button flex-1 justify-center" onClick={dismissLumiDialog}>Cảm ơn Lumi</button></div>}</section></div> : null}{sessionSummary ? <div className="modal-backdrop grid place-items-center p-4" role="presentation" onClick={dismissSessionSummary}><section className="session-summary-modal w-full max-w-sm border border-amber-200 bg-white p-5 text-slate-900 shadow-2xl dark:border-amber-300/25 dark:bg-slate-900 dark:text-slate-100" role="dialog" aria-modal="true" aria-labelledby="session-summary-title" onClick={(event) => event.stopPropagation()}><div className="text-center"><div className="text-4xl" aria-hidden="true">✅</div><p className="mt-3 text-xs font-black uppercase tracking-[.16em] text-amber-700 dark:text-amber-300">Tổng kết phiên học</p><h2 id="session-summary-title" className="mt-1 font-display text-xl font-black">Đã hoàn thành phiên {sessionSummary.sessionNumber} / {sessionSummary.totalSessions}</h2></div><div className="mt-4 grid gap-2 rounded-2xl bg-amber-50 p-4 text-sm dark:bg-amber-400/10"><p><strong>Môn:</strong> {sessionSummary.subject}</p><p><strong>Nội dung:</strong> {sessionSummary.topic}</p><p><strong>Thời gian:</strong> {sessionSummary.durationMinutes} phút</p></div><p className="mt-3 text-xs leading-5 text-slate-600 dark:text-slate-300">Hãy kiểm tra lại thông tin trên. Khi bắt đầu phiên mới, Lumi sẽ hỏi lại môn, nội dung và thời gian để lịch sử được ghi chính xác.</p><div className="mt-5 flex gap-2"><button type="button" className="secondary-button flex-1 justify-center" onClick={dismissSessionSummary}>Đóng</button><button type="button" className="primary-button flex-1 justify-center" onClick={openNextSessionPicker}>{mode === "focus" && pendingTransition === "focus" ? "Chọn phiên mới" : "Tiếp tục đến giờ nghỉ"}</button></div></section></div> : null}{subjectConfirmationOpen ? <div className="modal-backdrop grid place-items-center p-4" role="presentation" onClick={() => setSubjectConfirmationOpen(false)}><section className="subject-confirmation-modal w-full max-w-sm border border-emerald-200 bg-white p-5 text-slate-900 shadow-2xl dark:border-emerald-300/25 dark:bg-slate-900 dark:text-slate-100" role="dialog" aria-modal="true" aria-labelledby="subject-confirmation-title" onClick={(event) => event.stopPropagation()}><div className="text-center"><div className="text-4xl" aria-hidden="true">📚</div><p className="mt-3 text-xs font-black uppercase tracking-[.16em] text-emerald-700 dark:text-emerald-300">Trước khi học</p><h2 id="subject-confirmation-title" className="mt-1 font-display text-xl font-black">Hãy chọn môn học của phiên này.</h2><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">Bạn có thể chọn ngay tại đây hoặc mở thẳng khu “Bạn đang học gì?” trên trang Pomodoro.</p><button type="button" className="secondary-button mt-4 w-full justify-center" onClick={openSubjectPickerArea}>Mở phần chọn môn</button></div><label className="mt-4 block text-sm font-bold">Môn học<select className="field mt-2" value={subject} onChange={(event) => setSubject(event.target.value)} aria-label="Chọn môn học trước khi bắt đầu phiên"><option value="">-- Chọn môn --</option>{availableSubjects.map((item) => <option key={`confirm-${item}`} value={item}>{item}</option>)}<option value="__custom__">Nhập môn khác…</option></select></label>{subject === "__custom__" ? <input autoFocus className="field mt-2" value={subject === "__custom__" ? "" : subject} onChange={(event) => setSubject(event.target.value)} placeholder="Nhập tên môn học" aria-label="Nhập tên môn học khác" /> : null}<label className="mt-3 block text-sm font-bold">Nội dung phiên học<input className="field mt-2" value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="Ví dụ: Ôn chương 2, làm bài 15–20" aria-label="Nhập nội dung phiên học" /></label><p className="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-900 dark:bg-emerald-400/10 dark:text-emerald-100">Thời lượng theo nhịp Pomodoro đang chọn: <strong>{focus} phút</strong>. Muốn đổi thời lượng, hãy chỉnh ở phần Cài đặt Pomodoro.</p><div className="mt-5 flex gap-2"><button type="button" className="secondary-button flex-1 justify-center" onClick={() => setSubjectConfirmationOpen(false)}>Để sau</button><button type="button" className="primary-button flex-1 justify-center" onClick={confirmSubjectAndStart} disabled={!subject.trim() || subject === "__custom__" || !topic.trim()}>Xác nhận và bắt đầu</button></div></section></div> : null}{waterReminderVisible ? <div className="modal-backdrop grid place-items-center p-4" role="presentation" onClick={() => setWaterReminderVisible(false)}><section className="lumi-popup-modal w-full max-w-sm border border-sky-200 bg-white p-6 text-slate-900 shadow-2xl dark:border-sky-300/25 dark:bg-slate-900 dark:text-slate-100" role="dialog" aria-modal="true" aria-labelledby="lumi-water-title" onClick={(event) => event.stopPropagation()}><div className="text-center"><div className="text-5xl" aria-hidden="true">(´ー`)旦~~</div><p className="mt-3 text-xs font-black uppercase tracking-[.16em] text-sky-700 dark:text-sky-300">Lumi nhắc uống nước</p><h2 id="lumi-water-title" className="mt-1 font-display text-2xl font-black">Đến giờ uống nước rồi!</h2><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{waterMessage || LUMI_WATER_MESSAGE}</p></div><button type="button" className="primary-button mt-5 w-full justify-center" onClick={acknowledgeWater}>Đã uống 💧</button></section></div> : null}</>;
  ...
  return <><div className="space-y-5"><header className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-red-700 dark:text-red-300">Nhịp học tự quản lý</p><h1 className="mt-2 font-display text-4xl font-black">Pomodoro</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">Giữ nhịp tập trung, lưu môn học, nội dung và ghi chú của phiên. Bộ đếm vẫn chạy khi bạn đổi menu.</p></div><div className="flex flex-wrap gap-2"><button type="button" className="secondary-button" onClick={() => setCompactMode((value) => !value)}>{compactMode ? "Mở đầy đủ" : "Thu nhỏ"}</button><button type="button" className={deepFocusMode ? "primary-button" : "secondary-button"} onClick={() => setDeepFocusMode((value) => !value)}>{deepFocusMode ? "Thoát tập trung sâu" : "Tập trung sâu"}</button><button type="button" className="secondary-button" onClick={restore}>Khôi phục phiên</button><button type="button" className="secondary-button" onClick={() => onView("history")}>Lịch sử học</button>{onOpenDetached && !detachedWindow ? <button type="button" className="primary-button" onClick={onOpenDetached}>↗ Ghim ra màn hình</button> : null}{detachedWindow ? <button type="button" className="secondary-button" onClick={closeDetachedWindow}>Đóng cửa sổ</button> : null}</div></header>
    ...
    {deepFocusMode ? <section className="deep-focus-shell panel"><div className="deep-focus-shell__top"><div><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-700 dark:text-emerald-300">Chế độ tập trung sâu</p><p className="mt-2 text-sm font-bold text-[var(--scene-muted)]">Môn đang học: <strong className="text-[var(--scene-text)]">{subject || "Chưa chọn môn"}</strong></p></div><button type="button" className="secondary-button text-xs" onClick={() => setDeepFocusMode(false)}>Thoát</button></div><div className="deep-focus-shell__timer" role="timer" aria-live="off" aria-label={`Thời gian còn lại ${display}`}>{display}</div><p className="deep-focus-shell__progress">{sessionProgressLabel} · {labelForMode[mode]}</p><div className="deep-focus-shell__meter" aria-label={`Tiến độ phiên ${Math.round((activeMinutes * 60 - seconds) / Math.max(1, activeMinutes * 60) * 100)} phần trăm`}><span style={{ width: `${clamp((activeMinutes * 60 - seconds) / Math.max(1, activeMinutes * 60) * 100, 0, 100)}%` }} /></div><p className="mt-3 text-sm text-[var(--scene-muted)]">{lumiStatusMessage}</p><div className="mt-6 flex flex-wrap justify-center gap-2"><button type="button" className="primary-button min-w-48" onClick={begin}>{primaryActionLabel}</button><button type="button" className="secondary-button text-rose-700 dark:text-rose-300" onClick={endSessionCompletely}>Kết thúc hoàn toàn</button></div></section> : compactMode ? <section className="panel p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.16em] text-emerald-700">Pomodoro thu nhỏ</p><p className="font-mono text-5xl font-black text-slate-950 dark:text-white">{display}</p><p className="mt-1 text-xs font-bold text-slate-500">{sessionProgressLabel} · {lumiStatusLabel} · {labelForMode[mode]} · {subject || "Tự học"}</p><p className="mt-2 text-xs font-semibold text-slate-500">{timingSummary}</p></div><div className="flex flex-wrap gap-2"><button type="button" className="primary-button" onClick={begin}>{primaryActionLabel}</button><button type="button" className="secondary-button" onClick={reset}>Đặt lại</button><button type="button" className="secondary-button text-rose-700 dark:text-rose-300" onClick={endSessionCompletely}>Kết thúc hoàn toàn</button>{onOpenDetached && !detachedWindow ? <button type="button" className="secondary-button" onClick={onOpenDetached}>↗ Ghim ra màn hình</button> : null}</div></div><div className="mt-4 border-t border-slate-200 pt-4 dark:border-white/10">{supportButtons}</div></section> : <>
      <section className="grid gap-5 xl:grid-cols-[1.05fr_.95fr]"><section className="panel p-6 text-center"><p className="text-xs font-black uppercase tracking-[.18em] text-red-700 dark:text-red-300">{labelForMode[mode]}</p><p className="mt-2 text-sm font-black text-emerald-700 dark:text-emerald-300">{sessionProgressLabel}</p><p className="mt-4 font-mono text-[clamp(4rem,12vw,7rem)] font-black tracking-tight">{display}</p><div className="lumi-pomodoro-status-chip mx-auto mt-3" aria-live="polite">{lumiStatusLabel}</div><p className="mt-2 text-sm font-bold text-slate-500 dark:text-slate-300">{lumiStatusMessage}</p><p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-300">Nhịp đang chọn: {timingSummary}</p><div className="mt-6 flex flex-wrap justify-center gap-2"><button type="button" className="primary-button min-w-40" onClick={begin}>{primaryActionLabel}</button><button type="button" className="secondary-button" onClick={reset}>Đặt lại</button><button type="button" className="secondary-button text-rose-700 dark:text-rose-300" onClick={endSessionCompletely}>Kết thúc hoàn toàn</button><button type="button" className="secondary-button" onClick={() => setMiniPlayerPinned((value) => !value)}>{miniPlayerPinned ? "Bỏ ghim" : "Ghim mini"}</button>{onOpenDetached && !detachedWindow ? <button type="button" className="primary-button" onClick={onOpenDetached}>↗ Ghim ra màn hình</button> : null}</div><div className="mt-5 rounded-2xl bg-emerald-50 p-4 text-left dark:bg-emerald-400/10"><p className="text-xs font-black uppercase tracking-[.14em] text-emerald-800 dark:text-emerald-200">Lumi ở đây khi bạn cần</p><p className="mt-1 text-sm text-slate-600 dark:text-slate-200">Không có nhiệm vụ ngẫu nhiên hay thử thách ép buộc. Chỉ cần chọn lời hỗ trợ phù hợp với lúc này.</p><div className="mt-3">{supportButtons}</div></div></section>
      <section id="pomodoro-subject-picker" className="panel p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.16em] text-emerald-700">Trước khi bắt đầu</p><h2 className="mt-1 font-display text-2xl font-black">Bạn đang học gì?</h2></div><button type="button" className="text-sm font-bold text-red-700 underline" onClick={() => setShowSettings((value) => !value)}>{showSettings ? "Ẩn nhịp học" : "Chỉnh nhịp học"}</button></div>{recentCompletedSessions.length ? <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 dark:border-emerald-300/20 dark:bg-emerald-400/10"><div><p className="text-sm font-black text-emerald-950 dark:text-emerald-100">Tiếp tục phiên gần nhất</p><p className="mt-1 text-xs text-emerald-800 dark:text-emerald-200">Điền lại {recentCompletedSessions[0].subject || "môn học"} và nội dung vừa học; bạn vẫn có thể chỉnh trước khi xác nhận.</p></div><button type="button" className="secondary-button shrink-0 justify-center text-xs" onClick={reuseLastSession}>Dùng phiên gần nhất</button></div> : null}<div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">{activities.map((item) => <button key={item.id} type="button" className={`rounded-xl border p-3 text-left text-xs font-bold ${activity === item.id ? "border-red-500 bg-red-50 text-red-900 dark:bg-red-500/10 dark:text-red-100" : "border-slate-200 dark:border-white/10"}`} onClick={() => setActivity(item.id)}>{item.icon} {item.label}</button>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">Môn học<input className="field mt-2" list="pomodoro-subject-options" value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="Chọn hoặc nhập môn học" /><datalist id="pomodoro-subject-options">{availableSubjects.map((item) => <option key={item} value={item} />)}</datalist></label><label className="text-sm font-bold">Nội dung<input className="field mt-2" value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="Ví dụ: Nhà Trần" /></label></div><label className="mt-3 block text-sm font-bold">Ghi chú phiên học<textarea className="field mt-2 min-h-20" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Điều đã học, điểm cần xem lại hoặc cảm nhận ngắn." /></label>{showSettings ? <div className="mt-4 rounded-2xl border border-dashed border-red-200 p-4 dark:border-red-300/20"><div className="grid gap-2 sm:grid-cols-3">{presets.map((preset) => <button key={preset.label} type="button" className="rounded-xl border border-slate-200 p-3 text-left text-sm font-bold dark:border-white/10" onClick={() => choosePreset(preset)}>{preset.label}</button>)}{personalPresets.map((preset) => <button key={`personal-${preset.id}`} type="button" className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-left text-sm font-bold text-emerald-950 dark:border-emerald-300/20 dark:bg-emerald-400/10 dark:text-emerald-100" onClick={() => choosePreset(preset)}>Cá nhân {preset.label}</button>)}<button type="button" className="rounded-xl border border-dashed border-red-300 p-3 text-left text-sm font-bold text-red-800 dark:border-red-300/30 dark:text-red-200" onClick={saveCurrentPreset}>＋ Lưu nhịp hiện tại</button></div><div className="mt-3 grid grid-cols-3 gap-2"><label className="text-xs font-bold">Tập trung<input className="field mt-1" type="number" min="1" max="120" value={focus} onChange={(event) => { const value = clamp(Number(event.target.value) || 1, 1, 120); setFocus(value); if (!running && mode === "focus") setSeconds(value * 60); }} /></label><label className="text-xs font-bold">Nghỉ ngắn<input className="field mt-1" type="number" min="1" max="30" value={shortBreak} onChange={(event) => setShortBreak(clamp(Number(event.target.value) || 1, 1, 30))} /></label><label className="text-xs font-bold">Nghỉ dài<input className="field mt-1" type="number" min="1" max="45" value={longBreak} onChange={(event) => setLongBreak(clamp(Number(event.target.value) || 1, 1, 45))} /></label></div></div> : null}</section></section>
      ...
      <section className="panel p-5" aria-label="Cài đặt Lumi và Pomodoro"><div><p className="text-xs font-black uppercase tracking-[.16em] text-emerald-700">Lumi · Pomodoro</p><h2 className="mt-1 font-display text-xl font-black">Âm thanh tối giản, tập trung hơn</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-300">Pomodoro không phát nhạc nền. Chỉ có âm báo phiên học, âm báo nhắc nước do Ong chọn và giọng đọc Lumi khi được bật.</p></div><div className="mt-5 rounded-2xl border border-violet-200 bg-violet-50/70 p-4 dark:border-violet-300/15 dark:bg-violet-950/20"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.14em] text-violet-700 dark:text-violet-300">Âm báo phiên học</p><h3 className="mt-1 text-base font-black">Báo khi bắt đầu/kết thúc</h3><p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-300">Đây là âm báo bằng Web Audio, không phải nhạc nền. Có thể tăng đến x10, lặp tối đa 10 lần và kéo dài âm; hãy chọn mức vừa đủ nghe để bảo vệ tai.</p></div><div className="grid min-w-[min(100%,24rem)] gap-2 sm:grid-cols-3"><label className="text-xs font-bold text-violet-900 dark:text-violet-100">Âm lượng x{pomodoroAlertSettings.masterVolume.toFixed(1)}<input className="field mt-1 w-full" type="range" min="0" max="10" step="0.5" value={pomodoroAlertSettings.masterVolume} onChange={(event) => updatePomodoroAlertSettings({ ...pomodoroAlertSettings, masterVolume: Number(event.target.value) })} aria-label="Âm lượng âm báo Pomodoro" /><span className="text-[11px] font-semibold">x1–x10</span></label><label className="text-xs font-bold text-violet-900 dark:text-violet-100">Lặp {pomodoroAlertSettings.repeatCount} lần<input className="field mt-1 w-full" type="range" min="1" max="10" step="1" value={pomodoroAlertSettings.repeatCount} onChange={(event) => updatePomodoroAlertSettings({ ...pomodoroAlertSettings, repeatCount: Number(event.target.value) })} aria-label="Số lần lặp âm báo Pomodoro" /><span className="text-[11px] font-semibold">Tối đa 10 lần</span></label><label className="text-xs font-bold text-violet-900 dark:text-violet-100">Độ dài x{pomodoroAlertSettings.durationMultiplier.toFixed(1)}<input className="field mt-1 w-full" type="range" min="0.5" max="3" step="0.5" value={pomodoroAlertSettings.durationMultiplier} onChange={(event) => updatePomodoroAlertSettings({ ...pomodoroAlertSettings, durationMultiplier: Number(event.target.value) })} aria-label="Độ dài âm báo Pomodoro" /><span className="text-[11px] font-semibold">Âm ngân dài hơn</span></label></div></div><div className="mt-3 grid gap-2 md:grid-cols-2">{POMODORO_ALERT_EVENT_IDS.map((eventId) => { const eventSettings = pomodoroAlertSettings.events[eventId]; return <div key={eventId} className="rounded-xl border border-violet-100 bg-white/80 p-3 dark:border-white/10 dark:bg-white/[.035]"><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={eventSettings.enabled} onChange={(event) => updatePomodoroAlertSettings({ ...pomodoroAlertSettings, events: { ...pomodoroAlertSettings.events, [eventId]: { ...eventSettings, enabled: event.target.checked } } })} />{pomodoroAlertEventLabels[eventId]}</label><div className="mt-2 flex gap-2"><select className="field min-w-0 flex-1 text-xs" value={eventSettings.soundId} onChange={(event) => { if (isPomodoroAlertSoundId(event.target.value)) updatePomodoroAlertSettings({ ...pomodoroAlertSettings, events: { ...pomodoroAlertSettings.events, [eventId]: { ...eventSettings, soundId: event.target.value } } }); }} aria-label={`Âm báo ${pomodoroAlertEventLabels[eventId]}`}>{POMODORO_ALERT_SOUNDS.map((sound) => <option key={sound.id} value={sound.id}>{sound.label}</option>)}</select><button type="button" className="secondary-button shrink-0 px-3 text-xs" onClick={() => previewPomodoroAlert(eventId)}><span aria-hidden="true">🔊</span> Nghe thử</button></div></div>; })}</div></div><div className="mt-5 grid gap-5 lg:grid-cols-[1.05fr_.95fr]"><fieldset><legend className="text-sm font-bold">Mục tiêu và cách chuyển phiên</legend><label className="mt-3 block text-sm font-bold">Mục tiêu phiên học · {goalCompletedSessions}/{totalSessions}<input aria-label="Mục tiêu số phiên Pomodoro" className="field mt-2" type="number" min="1" max="12" value={totalSessions} onChange={(event) => { const next = clamp(Number(event.target.value) || 1, 1, 12); if (next === totalSessions) return; if (running && !window.confirm("Đổi mục tiêu sẽ dừng phiên hiện tại và bắt đầu lại từ phiên 1. Tiếp tục?")) return; const resetState = resetPomodoroForGoalChange(focus); setTotalSessions(next); setGoalCompletedSessions(resetState.completedFocusSessions); setMode(resetState.mode); setPendingTransition(resetState.pendingTransition); setSeconds(resetState.seconds); setRunning(resetState.running); setSessionStartedAt(resetState.sessionStartedAt); completionHandled.current = false; }} /></label><p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-300">Đạt đủ mục tiêu, Pomodoro dừng lại và chúc mừng Ong.</p><label className="mt-4 flex gap-2 text-sm"><input type="radio" name="auto" checked={autoAdvance} onChange={() => setAutoAdvance(true)} />Tự động chuyển</label><label className="mt-2 flex gap-2 text-sm"><input type="radio" name="auto" checked={!autoAdvance} onChange={() => setAutoAdvance(false)} />Tôi tự nhấn để chuyển</label></fieldset><fieldset><legend className="text-sm font-bold">Giọng đọc và nhắc uống nước</legend><label className="mt-3 flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={lumiSpeechEnabled} onChange={(event) => { setSpeechEnabledPreference(event.target.checked); saveLumiSpeechPreference(event.target.checked); onProfile({ ...profile, lumiSpeechEnabled: event.target.checked }, `Đã ${event.target.checked ? "bật" : "tắt"} giọng đọc Lumi.`); }} />Đọc lời thoại Lumi bằng giọng tiếng Việt</label><div className="mt-3 grid gap-3 sm:grid-cols-[minmax(9rem,.8fr)_minmax(8rem,1fr)]"><label className="text-sm font-bold">Cách nhắc<select className="field mt-1" value={lumiWaterSettings.scheduleMode ?? "interval"} onChange={(event) => updateLumiWaterSettings({ ...lumiWaterSettings, scheduleMode: event.target.value === "clock" ? "clock" : "interval" })}><option value="interval">Theo khoảng thời gian</option><option value="clock">Theo mốc giờ mỗi ngày</option></select></label>{lumiWaterSettings.scheduleMode === "clock" ? <div className="space-y-2"><div className="flex items-center justify-between gap-2"><span className="text-sm font-bold">Các mốc giờ trong ngày</span><button type="button" className="secondary-button px-3 py-1.5 text-xs" onClick={addDailyWaterTime}>+ Thêm mốc giờ</button></div>{(lumiWaterSettings.dailyTimes ?? [lumiWaterSettings.dailyTime ?? "09:00"]).map((time, index) => <div key={`water-time-${index}`} className="flex items-center gap-2"><input className="field min-w-0 flex-1" type="time" value={time} onChange={(event) => updateDailyWaterTime(index, event.target.value)} aria-label={`Mốc nhắc uống nước ${time}`} /><button type="button" className="secondary-button shrink-0 px-3 py-2 text-xs" onClick={() => removeDailyWaterTime(time)} aria-label={`Xóa mốc ${time}`}>Xóa</button></div>)}<p className="text-xs leading-5 text-slate-500 dark:text-slate-300">Bạn có thể đặt nhiều mốc, ví dụ 09:00, 11:30, 14:00, 16:30 và 20:00.</p></div> : <label className="text-sm font-bold">Mỗi<input className="field mt-1" type="number" min="5" max="180" value={lumiWaterSettings.intervalMinutes} onChange={(event) => updateLumiWaterSettings({ ...lumiWaterSettings, intervalMinutes: clamp(Number(event.target.value) || 5, 5, 180) })} />phút</label>}</div><label className="mt-3 block text-sm font-bold">Âm báo nhắc nước<select className="field mt-1" value={lumiWaterSettings.soundId} onChange={(event) => { if (isLumiWaterAlertSoundId(event.target.value)) updateLumiWaterSettings({ ...lumiWaterSettings, soundId: event.target.value }); }}>{LUMI_WATER_ALERT_SOUNDS.map((sound) => <option key={sound.id} value={sound.id}>{sound.label}</option>)}</select></label><div className="mt-3 grid gap-2 sm:grid-cols-3"><label className="text-xs font-bold">Âm lượng x{lumiWaterSettings.alertVolume.toFixed(1)}<input className="field mt-1 w-full" type="range" min="0" max="10" step="0.5" value={lumiWaterSettings.alertVolume} onChange={(event) => updateLumiWaterSettings({ ...lumiWaterSettings, alertVolume: Number(event.target.value) })} aria-label="Âm lượng âm báo uống nước" /><span className="text-[11px] font-semibold">x1–x10</span></label><label className="text-xs font-bold">Lặp {lumiWaterSettings.repeatCount} lần<input className="field mt-1 w-full" type="range" min="1" max="10" step="1" value={lumiWaterSettings.repeatCount} onChange={(event) => updateLumiWaterSettings({ ...lumiWaterSettings, repeatCount: Number(event.target.value) })} aria-label="Số lần lặp âm báo uống nước" /><span className="text-[11px] font-semibold">Chuỗi dài hơn</span></label><label className="text-xs font-bold">Độ dài x{lumiWaterSettings.durationMultiplier.toFixed(1)}<input className="field mt-1 w-full" type="range" min="0.5" max="3" step="0.5" value={lumiWaterSettings.durationMultiplier} onChange={(event) => updateLumiWaterSettings({ ...lumiWaterSettings, durationMultiplier: Number(event.target.value) })} aria-label="Độ dài âm báo uống nước" /><span className="text-[11px] font-semibold">Âm ngân dài hơn</span></label></div><button type="button" className="secondary-button mt-3" onClick={previewWaterAlert}><span aria-hidden="true">🔊</span> Nghe thử âm báo nhắc nước</button><label className="mt-3 block text-sm font-bold">Câu nhắc uống nước<input className="field mt-1" value={waterMessageDraft} onChange={(event) => setWaterMessageDraft(event.target.value)} placeholder="Ví dụ: Ong ơi, uống một ngụm nước nhé!" /></label><button type="button" className="secondary-button mt-3" onClick={saveWaterReminderMessage}>Lưu câu nhắc</button>{lumiWaterSettings.enabled ? <p className="mt-2 text-xs font-semibold text-slate-500 dark:text-slate-300">Nhắc mỗi {lumiWaterSettings.intervalMinutes} phút · Lần nhắc tiếp theo: {waterDisplay}. Chỉ âm báo đã chọn được phát, không có BGM.</p> : <p className="mt-2 text-xs font-semibold text-slate-500 dark:text-slate-300">Nhắc uống nước đang tắt.</p>}</fieldset></div></section>
    </>}
    {lumiPopups}
