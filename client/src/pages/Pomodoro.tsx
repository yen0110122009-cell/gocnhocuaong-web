...
import { DEFAULT_LUMI_WATER_MESSAGE, readLumiDialogueLines, readLumiSpeechPreference, readLumiWaterMessage, saveLumiDialogueLines, saveLumiSpeechPreference, saveLumiWaterMessage } from "../lib/lumiPreferences";
...
export default function Pomodoro({ profile, config, onProfile, onView, isVisible = true }: Props) {
  ...
  const [customDialogueLines, setCustomDialogueLines] = useState<string[]>(() => readLumiDialogueLines());
  ...
  const lumiKaomoji = mode === "focus" && running ? "٩(ˊᗜˋ*)و" : mode === "focus" ? "[(－－)]..zzZ" : "(´ー`)旦~~";
  ...
  function addDialogueLine() {
    const value = newDialogueLine.trim();
    if (!value) return;
    const next = saveLumiDialogueLines([...customDialogueLines, value]);
    setCustomDialogueLines(next);
    setNewDialogueLine("");
    toast.success("Đã thêm câu thoại Lumi.");
  }
  function updateDialogueLine(index: number, value: string) {
    const next = saveLumiDialogueLines(customDialogueLines.map((line, lineIndex) => lineIndex === index ? value : line));
    setCustomDialogueLines(next);
  }
  function removeDialogueLine(index: number) {
    setCustomDialogueLines(saveLumiDialogueLines(customDialogueLines.filter((_, lineIndex) => lineIndex !== index)));
  }

  function acknowledgeWater() {
    const praise = "Giỏi lắm! Cảm ơn Ong đã uống nước. Mình cùng tiếp tục nhẹ nhàng nhé!";
    setWaterReminderVisible(false);
    setWaterFeedback("💧 Lumi thả tim: Giỏi lắm, Ong đã uống nước rồi!");
    speakLumi(praise);
    setWaterSecondsRemaining(lumiWaterSettings.intervalMinutes * 60);
    window.clearTimeout(waterFeedbackTimeoutRef.current);
    waterFeedbackTimeoutRef.current = window.setTimeout(() => setWaterFeedback(null), 4_000);
  }

  function openLumiDialog() { setLumiDialogResponse(null); setShowLumiDialog(true); speakLumi("Lumi chào Ong. Hôm nay Ong cảm thấy thế nào?"); }
  ...
  function chooseLumiFeeling(choice: "tired" | "motivation" | "hug" | "ready") {
    const responses = {
      tired: { group: "An ủi & Vỗ về", kaomoji: "(｡•́︿•̀｡)", text: "Mình chậm lại một nhịp nhé. Ong không cần hoàn hảo; chỉ cần chăm sóc bản thân và làm một bước thật nhỏ." },
      motivation: { group: "Cố gắng & Động viên", kaomoji: "٩(ˊᗜˋ*)و", text: "Lumi tin Ong làm được. Mình bắt đầu bằng năm phút thôi, rồi để đà tiến bộ dẫn đường." },
      hug: { group: "Ôm ấp & Yêu thương", kaomoji: "(っ´▽`)っ", text: "Gửi Ong một cái ôm thật ấm. Ong đã cố gắng nhiều rồi, mình cùng học theo nhịp vừa sức nhé." },
      ready: { group: "Tình bạn & Đồng hành", kaomoji: "(*•̀ᴗ•́*)و ̑̑", text: "Lumi sẵn sàng ngồi cạnh Ong. Một phiên, một mục tiêu, mình cùng làm nhé!" },
    }[choice];
    setLumiDialogResponse(responses);
    speakLumi(responses.text);
  }
  ...
  function createSupport(kind: LumiSupportKind) {
    ...
    const customLine = customDialogueLines[Math.floor(Math.random() * customDialogueLines.length)];
    return { kind, text: line?.text || customLine || custom?.text || state?.description || (kind === "comfort" ? "Không sao nếu hôm nay chậm. Mình cùng quay lại bằng một bước vừa sức nhé." : "Mỗi phút bạn đang ở lại với việc học đều đáng được ghi nhận.") };
  }
  ...
  const lumiPopups = <>{showLumiDialog ? <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/45 p-4 backdrop-blur-[2px]" role="presentation" onClick={dismissLumiDialog}><section className="w-full max-w-md rounded-3xl border border-emerald-200 bg-white p-6 text-slate-900 shadow-2xl dark:border-emerald-300/25 dark:bg-slate-900 dark:text-slate-100" role="dialog" aria-modal="true" aria-labelledby="lumi-checkin-title" onClick={(event) => event.stopPropagation()}><div className="text-center"><div className="text-5xl" aria-hidden="true">{lumiDialogResponse?.kaomoji ?? "٩(◕‿◕｡)۶"}</div><p className="mt-3 text-xs font-black uppercase tracking-[.16em] text-emerald-700 dark:text-emerald-300">Lumi hỏi thăm</p><h2 id="lumi-checkin-title" className="mt-1 font-display text-2xl font-black">{lumiDialogResponse ? lumiDialogResponse.group : "Hôm nay Ong cảm thấy thế nào?"}</h2><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{lumiDialogResponse?.text ?? "Lumi ở đây để lắng nghe. Chọn một điều gần với cảm xúc của Ong nhé."}</p></div>{!lumiDialogResponse ? <div className="mt-5 grid grid-cols-2 gap-2"><button type="button" className="secondary-button justify-center text-sm" onClick={() => chooseLumiFeeling("tired")}>Mệt mỏi</button><button type="button" className="secondary-button justify-center text-sm" onClick={() => chooseLumiFeeling("motivation")}>Thiếu động lực</button><button type="button" className="secondary-button justify-center text-sm" onClick={() => chooseLumiFeeling("hug")}>Cần cái ôm</button><button type="button" className="secondary-button justify-center text-sm" onClick={() => chooseLumiFeeling("ready")}>Sẵn sàng học</button></div> : <div className="mt-5 flex gap-2"><button type="button" className="secondary-button flex-1 justify-center" onClick={() => setLumiDialogResponse(null)}>Hỏi lại</button><button type="button" className="primary-button flex-1 justify-center" onClick={dismissLumiDialog}>Cảm ơn Lumi</button></div>}</section></div> : null}{waterReminderVisible ? <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/45 p-4 backdrop-blur-[2px]" role="presentation" onClick={() => setWaterReminderVisible(false)}><section className="w-full max-w-sm rounded-3xl border border-sky-200 bg-white p-6 text-slate-900 shadow-2xl dark:border-sky-300/25 dark:bg-slate-900 dark:text-slate-100" role="dialog" aria-modal="true" aria-labelledby="lumi-water-title" onClick={(event) => event.stopPropagation()}><div className="text-center"><div className="text-5xl" aria-hidden="true">(´ー`)旦~~</div><p className="mt-3 text-xs font-black uppercase tracking-[.16em] text-sky-700 dark:text-sky-300">Lumi nhắc uống nước</p><h2 id="lumi-water-title" className="mt-1 font-display text-2xl font-black">Đến giờ uống nước rồi!</h2><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">Ong uống một ngụm nước nhé. Giữ cơ thể đủ nước sẽ giúp mình học dễ chịu hơn.</p></div><button type="button" className="primary-button mt-5 w-full justify-center" onClick={acknowledgeWater}>Đã uống 💧</button></section></div> : null}</>;
  ...
  if (!isVisible) {
    ...
    return <><aside className="fixed z-[80] w-[min(92vw,22rem)] touch-none select-none rounded-2xl border border-emerald-300/50 bg-[linear-gradient(135deg,#fffdf8_0%,#eff9f0_100%)] p-4 text-slate-900 shadow-2xl" style={{ left: `${miniPlayerPosition.x}%`, top: `${miniPlayerPosition.y}%`, transform: "translate(-50%, -50%)" }} aria-label="Pomodoro Lumi đang chạy" onPointerDown={startWidgetDrag} onPointerMove={moveWidget} onPointerUp={stopWidgetDrag} onPointerCancel={stopWidgetDrag}><div className="flex items-start gap-3"><button type="button" className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-emerald-100 text-2xl shadow-inner" aria-label="Mở hộp thoại hỏi thăm của Lumi" onClick={() => { openLumiDialog(); }}>{lumiKaomoji}</button><div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[.16em] text-emerald-800">Lumi · Pomodoro</p><p className="mt-1 font-mono text-3xl font-black">{display}</p><p className="truncate text-xs font-semibold text-slate-600">{labelForMode[mode]} · {subject || "Tự học"}</p></div></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" className="primary-button flex-1" onClick={begin}>{running ? "Tạm dừng" : "Tiếp tục"}</button><button type="button" className="secondary-button text-xs" onClick={() => onView("pomodoro")}>Mở Pomodoro</button></div>{waterFeedback ? <p className="mt-2 text-xs font-black text-rose-600">{waterFeedback}</p> : null}</aside>{lumiPopups}</>;
  }
