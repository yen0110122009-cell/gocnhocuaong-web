...
export default function AIDataImport({ profile, onProfile, onView }: Props) {
  ...
  const [purpose, setPurpose] = useState("Ôn tập và củng cố kiến thức");
  const [grade, setGrade] = useState("Lớp 8");
  const [topic, setTopic] = useState("");
  const [course, setCourse] = useState("");
  const [educationLevel, setEducationLevel] = useState<EducationLevel>("THCS");
  const [difficulty, setDifficulty] = useState<"Cơ bản" | "Trung bình" | "Nâng cao">("Trung bình");
  ...
  const options = useMemo<AiImportOptions>(() => ({ target, questionType, quantity, customQuantity, title, subject, purpose, grade, topic, course, educationLevel, difficulty, extraRequest, quizMode, timerMode, durationMinutes }), [target, questionType, quantity, customQuantity, title, subject, purpose, grade, topic, course, educationLevel, difficulty, extraRequest, quizMode, timerMode, durationMinutes]);
  const makePrompt = () => { setPrompt(buildExternalAiPrompt(options)); toast.success("Đã tạo prompt cho AI bên ngoài."); };
  const validate = () => { const result = validateExternalAiData(rawData); setValidated(result); if (result.metadata.title) setTitle(result.metadata.title); if (result.metadata.subject) setSubject(result.metadata.subject); if (result.metadata.purpose) setPurpose(result.metadata.purpose); if (result.metadata.grade) setGrade(result.metadata.grade); if (result.metadata.topic) setTopic(result.metadata.topic); if (result.metadata.course) setCourse(result.metadata.course); if (result.metadata.educationLevel) setEducationLevel(result.metadata.educationLevel); if (result.metadata.difficulty) setDifficulty(result.metadata.difficulty); result.valid ? toast.success(`Dữ liệu hợp lệ: ${result.questions.length} câu và đã nhận đủ metadata.`) : toast.error(`${result.errors.length} lỗi cần sửa trước khi nhập.`); };
  ...
  const inspectHistory = (item: ProfileState["aiImportHistory"][number]) => { setTitle(item.title); setSubject(item.subject ?? ""); setPurpose(item.purpose ?? ""); setGrade(item.grade ?? ""); setTopic(item.topic ?? ""); setCourse(item.course ?? ""); if (item.educationLevel) setEducationLevel(item.educationLevel); if (item.difficulty) setDifficulty(item.difficulty); setPrompt(item.prompt); setRawData(item.rawData); setValidated(null); toast.success("Đã nạp lại dữ liệu nhập và toàn bộ metadata để xem hoặc tạo lại."); };
  ...
  const createLearningContent = () => {
    ...
    const returned = result.metadata;
    const contentMetadata = { title: returned.title || title, subject: returned.subject || subject, purpose: returned.purpose || purpose, grade: returned.grade || grade, topic: returned.topic || topic, course: returned.course || course, educationLevel: returned.educationLevel || educationLevel, difficulty: returned.difficulty || difficulty };
    const set = shouldCards ? convertImportToFlashcards(result, contentMetadata) : undefined;
    const quiz = shouldQuiz ? convertImportToQuiz(result, { ...contentMetadata, mode: quizMode, timerMode: quizMode === "review" ? "unlimited" : timerMode, durationMinutes: Math.max(1, durationMinutes) }) : undefined;
    const record = { id: uid(), title: contentMetadata.title, subject: contentMetadata.subject, purpose: contentMetadata.purpose, grade: contentMetadata.grade, topic: contentMetadata.topic, course: contentMetadata.course, educationLevel: contentMetadata.educationLevel, difficulty: contentMetadata.difficulty, createdAt: now, target, questionCount: quiz?.questions.length ?? 0, flashcardCount: set?.cards.length ?? 0, prompt, rawData, quizId: quiz?.id, flashcardSetId: set?.id };
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="panel p-6">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label className="text-sm font-bold">Mục đích học<input aria-label="Mục đích học AI" value={purpose} onChange={(e) => setPurpose(e.target.value)} className="input mt-2 w-full" placeholder="Ví dụ: chuẩn bị kiểm tra giữa kỳ" /></label>
            <label className="text-sm font-bold">Lớp học<input aria-label="Lớp học AI" value={grade} onChange={(e) => setGrade(e.target.value)} className="input mt-2 w-full" placeholder="Ví dụ: Lớp 8" /></label>
            <label className="text-sm font-bold">Mức độ<select aria-label="Mức độ AI" value={difficulty} onChange={(e) => setDifficulty(e.target.value as typeof difficulty)} className="select mt-2 w-full"><option>Cơ bản</option><option>Trung bình</option><option>Nâng cao</option></select></label>
        ...
        <section className="panel p-6">
          ...
          <label className="mt-5 flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-amber-300 bg-amber-50/50 p-4 text-sm font-bold dark:border-amber-500/30 dark:bg-amber-400/5"><FileUp className="h-5 w-5 text-amber-700" />{documentName || "Chọn tên tài liệu (không tải tự động sang AI)"}<input className="sr-only" type="file" accept=".json,.txt,.md" onChange={async (e) => { const file = e.target.files?.[0]; if (!file) return; const extension = file.name.toLowerCase().split(".").pop(); if (!extension || !["json", "txt", "md"].includes(extension)) { toast.error("AI Studio hiện nhận JSON, TXT hoặc Markdown."); return; } setDocumentName(file.name); setRawData(await file.text()); setValidated(null); toast.success(`Đã nạp ${file.name} vào vùng dữ liệu AI.`); }} /></label>
