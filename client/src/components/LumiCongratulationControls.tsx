...
export function LumiCongratulationControls({ profile, emotion, emotionLabel, onProfile }: Props) {
  ...
  function save() {
    ...
    setDraft(""); setDraftAudioUrl(undefined); setEditingId(null); setRecordingError(""); setRecordingStatus("Đã lưu lời chúc và bản thu Lumi.");
  }

  function beginEdit(message: LumiCongratulationMessage) {
    setEditingId(message.id); setDraft(message.text); setDraftAudioUrl(message.audioUrl); setRecordingError(""); setRecordingStatus("");
  }

  function cancelEdit() { setEditingId(null); setDraft(""); setDraftAudioUrl(undefined); setRecordingError(""); setRecordingStatus(""); }
  ...
  async function toggleRecording() {
    ...
    if (recording) { recorderRef.current?.stop(); setRecording(false); setRecordingStatus("Đang lưu bản ghi vừa thu…"); return; }
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try { setDraftAudioUrl(await toDataUrl(new Blob(chunksRef.current, { type: recorder.mimeType || "audio/webm" }))); setRecordingStatus("Đã ghi xong. Ong có thể nghe thử hoặc lưu bản thu."); }
        catch (error) { setRecordingError(error instanceof Error ? error.message : "Không thể lưu bản ghi."); }
      };
      recorderRef.current = recorder; recorder.start(); setRecording(true); setRecordingStatus("Đang ghi âm. Nhấn Dừng ghi âm khi hoàn tất.");
