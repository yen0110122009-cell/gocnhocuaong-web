...
type TimelinePoint = { label: string; events: number; characters: number };
...
function buildTimeline(config: AppConfig): TimelinePoint[] {
  ...
  const points = Array.from({ length: 6 }, (_, index) => {
    ...
    return { key: monthKey(date), label: monthLabel(date), events: 0, characters: 0 };
  });
