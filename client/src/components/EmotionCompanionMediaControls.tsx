...
export function EmotionCompanionMediaControls({ profile, emotion, onProfile }: Props) {
  ...
  const updateRecordingImage = (recordingId: string, imageUrl: string, message = "Đã đổi ảnh đại diện cho bản thu Lumi.") => {
    updateVoiceRecordings(voiceRecordings.map((voice) => voice.id === recordingId ? { ...voice, imageUrl } : voice), media.favoriteLumiVoiceId, message);
  };
  ...
  async function uploadFile(file: File, kind: MediaKind) {
    ...
    try {
      ...
      if (kind === "lumi-voice") {
        ...
        const item: LumiVoiceRecording = { id, url: result.url, label: file.name.replace(/\.[^/.]+$/, "").slice(0, 80) || "Bản thu Lumi", createdAt: new Date().toISOString(), imageUrl: media.lumiImageUrl || getDefaultLumiImage(emotion) };
  ...
  async function uploadRecordingImage(file: File, recordingId: string) {
    const token = getToken();
    ...
    try {
      ...
      updateRecordingImage(recordingId, result.url, "Đã đổi ảnh đi kèm bản thu Lumi.");
  ...
  return <section className="relative z-10 mt-4 rounded-2xl border border-[#2e7d32]/20 bg-white/85 p-4 shadow-sm" aria-label="Ảnh và giọng Lumi theo cảm xúc">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-3">
      ...
      <div className="rounded-xl border border-violet-200 bg-violet-50/70 p-3 sm:col-span-2"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-black text-violet-800">Bộ sưu tập ảnh–giọng Lumi</p><p className="mt-1 text-xs leading-5 text-violet-800">Mỗi thẻ là một cặp ảnh–giọng riêng. Nhấn nút phát ngay trên ảnh để nghe thử, hoặc thay ảnh chỉ cho bản thu đó.</p></div>{voiceRecordings.length ? <span className="rounded-full border border-violet-200 bg-white px-2.5 py-1 text-[10px] font-black text-violet-800">{voiceRecordings.length} bản thu</span> : null}</div><div className="mt-3 flex flex-wrap gap-2"><VoiceInput /><button type="button" onClick={() => void toggleRecord()} disabled={Boolean(busy)} className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold text-white ${recording ? "bg-red-600" : "bg-violet-700"}`}><Mic className="h-4 w-4" />{recording ? "Dừng ghi" : "Ghi âm"}</button></div>{voiceRecordings.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{voiceRecordings.map((item) => { const isFavorite = favoriteVoice?.id === item.id; const linkedImage = item.imageUrl || media.lumiImageUrl || getDefaultLumiImage(emotion); const preview = () => { const audio = new Audio(item.url); audio.volume = (profile.audioMixer?.lumi ?? 75) / 100; void audio.play(); }; return <article key={item.id} className={`overflow-hidden rounded-xl border p-2 shadow-sm ${isFavorite ? "border-amber-300 bg-amber-50" : "border-violet-100 bg-white/85"}`}><div className="group relative aspect-[4/3] overflow-hidden rounded-lg bg-violet-100"><img src={linkedImage} alt={`Ảnh Lumi gắn với ${item.label}`} className="h-full w-full object-cover object-top transition duration-200 group-hover:scale-[1.03]" /><button type="button" onClick={preview} className="absolute inset-0 m-auto flex h-12 w-12 items-center justify-center rounded-full border border-white/70 bg-violet-800/90 text-white shadow-lg transition hover:scale-105" aria-label={`Nghe thử ${item.label}`}><Play className="ml-0.5 h-5 w-5" fill="currentColor" /></button>{isFavorite ? <span className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full bg-amber-400 px-2 py-1 text-[10px] font-black text-amber-950"><Star className="h-3 w-3" fill="currentColor" />Bản thu ưu tiên</span> : null}</div><div className="mt-2"><input value={item.label} maxLength={80} aria-label="Tên bản thu Lumi" onChange={(event) => updateVoiceRecordings(voiceRecordings.map((voice) => voice.id === item.id ? { ...voice, label: event.target.value } : voice), media.favoriteLumiVoiceId)} className="w-full rounded-lg border border-violet-100 bg-white px-2 py-1.5 text-xs font-bold text-violet-950" /><p className="mt-1 text-[10px] font-medium text-violet-700">{new Date(item.createdAt).toLocaleDateString("vi-VN")}</p></div><div className="mt-2 flex flex-wrap gap-1.5"><label className="inline-flex cursor-pointer items-center gap-1 rounded-lg border border-violet-200 bg-white px-2 py-1.5 text-[10px] font-black text-violet-800 hover:bg-violet-50"><ImagePlus className="h-3.5 w-3.5" />{busy === "lumi-image" ? "Đang tải…" : "Đổi ảnh"}<input className="sr-only" type="file" accept="image/png,image/jpeg,image/webp,image/gif" disabled={Boolean(busy)} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadRecordingImage(file, item.id); event.currentTarget.value = ""; }} /></label><button type="button" onClick={() => updateRecordingImage(item.id, media.lumiImageUrl || getDefaultLumiImage(emotion), "Đã dùng lại ảnh Lumi của cảm xúc này cho bản thu.")} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[10px] font-black text-slate-700 hover:bg-slate-50">Ảnh cảm xúc</button><button type="button" onClick={() => updateVoiceRecordings(voiceRecordings, item.id, `Đã chọn “${item.label}” làm bản thu ưu tiên.`)} className={`inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-[10px] font-black ${isFavorite ? "bg-amber-400 text-amber-950" : "border border-amber-200 bg-white text-amber-700 hover:bg-amber-50"}`} aria-label={`Chọn ${item.label} làm yêu thích`}><Star className="h-3.5 w-3.5" fill={isFavorite ? "currentColor" : "none"} />{isFavorite ? "Ưu tiên" : "Chọn"}</button><button type="button" onClick={() => removeVoice(item.id)} className="rounded-lg border border-red-200 bg-white p-1.5 text-red-700 hover:bg-red-50" aria-label={`Gỡ ${item.label}`}><Trash2 className="h-3.5 w-3.5" /></button></div></article>; })}</div> : <p className="mt-3 text-xs font-bold text-violet-700">Chưa có bản thu cá nhân. Khi có, mỗi bản thu sẽ hiện thành một thẻ ảnh–giọng riêng.</p>}</div>
    </div>
  </section>;
}
