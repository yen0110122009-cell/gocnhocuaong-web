import { Copy, Download, Eye, EyeOff, GripVertical, ImagePlus, LoaderCircle, Mic, Play, Search, Star, Trash2, Upload } from "lucide-react";
...
export function EmotionCompanionMediaControls({ profile, emotion, onProfile }: Props) {
  ...
  const ImageInput = ({ kind, label }: { kind: "mascot-image" | "lumi-image"; label: string }) => { const loading = busy === kind; return <label aria-busy={loading} className={`inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-xl border border-[#2e7d32]/20 bg-white px-3 py-2 text-xs font-bold text-[#2e7d32] transition hover:bg-[#eff9ef] active:scale-[.98] ${loading ? "pointer-events-none animate-pulse opacity-70" : ""}`}><span className="sr-only">{loading ? "Đang tải ảnh lên" : label}</span>{loading ? <LoaderCircle className="h-4 w-4 animate-spin" aria-hidden="true" /> : <ImagePlus className="h-4 w-4" aria-hidden="true" />}{loading ? "Đang tải ảnh…" : label}<input className="sr-only" type="file" accept="image/png,image/jpeg,image/webp,image/gif" disabled={Boolean(busy)} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadFile(file, kind); event.currentTarget.value = ""; }} /></label>; };
