...
export function EmotionCompanionMediaControls({ profile, emotion, onProfile }: Props) {
  ...
  const draftQuery = trpc.study.profile.getCompanionDraft.useQuery({ token }, { enabled: Boolean(token), staleTime: 15_000 });
  const saveDraftMutation = trpc.study.profile.saveCompanionDraft.useMutation();
  const restoreDraftMutation = trpc.study.profile.restoreCompanionDraft.useMutation();
  ...
  const [mergeChoices, setMergeChoices] = useState<Partial<Record<EmotionThemeId, DraftSource>>>({});
  ...
  const localDraftPayload = useMemo<CompanionDraftPayload | null>(() => availableDraft ? { companionEmotionMedia: availableDraft.companionEmotionMedia, savedAt: availableDraft.savedAt } : null, [availableDraft]);
  const cloudDraftPayload = useMemo<CompanionDraftPayload | null>(() => parseCompanionDraft(draftQuery.data?.current?.data), [draftQuery.data?.current?.data]);
  const mergeDiffs = useMemo(() => companionDraftDiff(localDraftPayload, cloudDraftPayload), [cloudDraftPayload, localDraftPayload]);
  useEffect(() => {
    if (!token || !draftQuery.data?.current?.deviceLabel || draftQuery.data.current.deviceLabel === deviceLabel) return;
    try {
      const key = "companion-media-devices:v1";
      const known = JSON.parse(localStorage.getItem(key) || "[]") as unknown;
      const knownDevices = Array.isArray(known) ? known.filter((item): item is string => typeof item === "string") : [];
      if (!knownDevices.includes(deviceLabel)) setNewDeviceNotice(true);
      if (!knownDevices.includes(deviceLabel)) localStorage.setItem(key, JSON.stringify([...knownDevices, deviceLabel].slice(-8)));
    } catch { setNewDeviceNotice(true); }
  }, [deviceLabel, draftQuery.data?.current?.deviceLabel, token]);
  function dismissNewDeviceNotice() {
    setNewDeviceNotice(false);
    try {
      const key = "companion-media-devices:v1";
      const known = JSON.parse(localStorage.getItem(key) || "[]") as unknown;
      const knownDevices = Array.isArray(known) ? known.filter((item): item is string => typeof item === "string") : [];
  ...
  function openMergePanel() {
    if (!localDraftPayload || !cloudDraftPayload || !mergeDiffs.length) return;
    setMergeChoices(Object.fromEntries(mergeDiffs.map((item) => [item.emotion, "merge" as DraftSource])) as Partial<Record<EmotionThemeId, DraftSource>>);
    setShowMergePanel(true);
  }
  function applyMerge() {
    if (!token || !localDraftPayload || !cloudDraftPayload || !mergeDiffs.length) return;
    const merged = mergeCompanionDrafts(localDraftPayload, cloudDraftPayload, mergeChoices);
    if (merged.companionEmotionMedia) onProfile({ ...profile, companionEmotionMedia: merged.companionEmotionMedia }, "Đã hợp nhất nháp cục bộ và nháp tài khoản.");
    setSyncStatus("syncing");
    saveDraftMutation.mutate({ token, data: JSON.stringify(merged), expectedVersion: draftQuery.data?.current?.version ?? 0, deviceLabel }, {
      onSuccess: () => { setSyncStatus("synced"); setShowMergePanel(false); dismissNewDeviceNotice(); void draftQuery.refetch(); setCollectionMessage("Đã hợp nhất nháp an toàn và lưu thành phiên bản mới."); },
      onError: (error) => { setSyncStatus(error.message.includes("thiết bị khác") ? "conflict" : "error"); setCollectionMessage("Không thể hợp nhất vì nháp tài khoản đã thay đổi. Hãy tải lại khác biệt rồi thử lại."); },
    });
  }
  useEffect(() => {
    let cancelled = false;
    ...
    try {
      const draft = { savedAt, emotion, recording, companionEmotionMedia: profile.companionEmotionMedia };
      localStorage.setItem(draftStorageKey, JSON.stringify(draft));
      setDraftSavedAt(savedAt);
      setAvailableDraft({ savedAt, companionEmotionMedia: draft.companionEmotionMedia });
    } catch { /* localStorage may be unavailable in private browsing */ }
    if (!token || !syncInitializedRef.current) {
    ...
    const timer = window.setTimeout(() => {
      if (cancelled) return;
      saveDraftMutation.mutate({ token, data: JSON.stringify({ savedAt, emotion, recording, companionEmotionMedia: profile.companionEmotionMedia }), expectedVersion: draftQuery.data?.current?.version ?? 0, deviceLabel }, {
        onSuccess: () => { if (!cancelled) { setSyncStatus("synced"); void draftQuery.refetch(); } },
        onError: (error) => { if (!cancelled) setSyncStatus(error.message.includes("thiết bị khác") ? "conflict" : "error"); },
      });
    }, 900);
  ...
  }, [colorFilter, emotionFilter, libraryItems, librarySort, search]);
  ...
  async function uploadFile(file: File, kind: MediaKind) {
    ...
    try {
      ...
      const mediaUrl = isGitHubPages ? dataUrl : resolveMediaUrl((await (async () => {
        ...
        return upload.mutateAsync({ token, fileName: file.name, contentType: file.type as "audio/webm", dataUrl, mediaType: kind });
      })()).url);
  ...
  return <section className="relative z-10 mt-4 rounded-2xl border border-[#2e7d32]/20 bg-white/85 p-4 shadow-sm" aria-label="Bản thu giọng Lumi theo cảm xúc">
    {showOnboarding ? <div className="mb-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-950" role="note"><div className="flex items-start justify-between gap-3"><div><p className="font-black">Bắt đầu với Lumi trong 3 bước</p><p className="mt-1 leading-5">① Ghi âm lời động viên bằng micro của bạn. ② Chọn cảm xúc và nhấn nút phát để nghe lại bất cứ lúc nào.</p></div><button type="button" onClick={() => { setShowOnboarding(false); try { localStorage.setItem("companion-media-onboarding-v1", "done"); } catch { /* ignore */ } }} className="shrink-0 rounded-lg border border-emerald-300 bg-white px-2 py-1 text-[11px] font-black text-emerald-800">Đã hiểu</button></div></div> : null}
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2 text-[10px] font-bold" role="status" aria-live="polite"><span className="text-slate-500">{draftSavedAt ? `Nháp cục bộ đã lưu ${new Date(draftSavedAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })}.` : "Nháp cục bộ đang sẵn sàng."}</span>{token ? <span className={syncStatus === "synced" ? "text-emerald-700" : syncStatus === "conflict" || syncStatus === "error" ? "text-rose-700" : "text-sky-700"}>{syncStatus === "syncing" ? "Đang đồng bộ…" : syncStatus === "synced" ? "Đã đồng bộ tài khoản" : syncStatus === "conflict" ? "Có thay đổi trên thiết bị khác" : syncStatus === "error" ? "Đồng bộ lỗi · vẫn giữ nháp cục bộ" : "Đang chờ đồng bộ"}</span> : <span className="text-amber-700">Đăng nhập để đồng bộ đa thiết bị</span>}</div>
    ...
    {draftQuery.data?.current?.data ? <div className="mb-3 rounded-xl border border-indigo-200 bg-indigo-50 p-3 text-xs text-indigo-950" role="region" aria-label="Nháp đã đồng bộ trên tài khoản"><div className="flex flex-wrap items-center justify-between gap-2"><span><strong>Nháp tài khoản phiên bản {draftQuery.data.current.version}</strong> · {draftQuery.data.current.deviceLabel || "thiết bị khác"}</span><button type="button" onClick={() => { try { const cloud = JSON.parse(draftQuery.data?.current?.data || "{}"); if (cloud.companionEmotionMedia) onProfile({ ...profile, companionEmotionMedia: cloud.companionEmotionMedia }, "Đã khôi phục nháp từ tài khoản."); } catch { setSyncStatus("error"); } }} className="rounded-lg bg-indigo-700 px-3 py-1.5 font-black text-white">Dùng nháp tài khoản</button></div><button type="button" onClick={() => setShowCloudHistory((value) => !value)} className="mt-2 font-black text-indigo-800 underline underline-offset-2">{showCloudHistory ? "Ẩn lịch sử phiên bản" : "Xem lịch sử phiên bản"}</button>{showCloudHistory ? <div className="mt-2 grid gap-2">{draftQuery.data.versions.map((version) => <div key={version.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-indigo-100 bg-white p-2"><span>Phiên bản {version.version} · {version.deviceLabel || "thiết bị"} · {new Date(version.createdAt).toLocaleString("vi-VN")}</span><button type="button" onClick={() => { if (!token || !window.confirm(`Khôi phục phiên bản ${version.version}? Nháp hiện tại sẽ được lưu thành một phiên bản mới trước khi thay thế.`)) return; restoreDraftMutation.mutate({ token, version: version.version, expectedVersion: draftQuery.data?.current?.version ?? 0, deviceLabel }, { onSuccess: (result) => { try { const restored = JSON.parse(result.data) as { companionEmotionMedia?: ProfileState["companionEmotionMedia"] }; if (restored.companionEmotionMedia) onProfile({ ...profile, companionEmotionMedia: restored.companionEmotionMedia }, `Đã khôi phục phiên bản ${version.version}.`); } catch { setSyncStatus("error"); } void draftQuery.refetch(); setSyncStatus("synced"); }, onError: (error) => { setSyncStatus(error.message.includes("thiết bị khác") ? "conflict" : "error"); } }); }} disabled={restoreDraftMutation.isPending} className="rounded-lg border border-indigo-300 px-2 py-1 font-black text-indigo-800 disabled:opacity-50">{restoreDraftMutation.isPending ? "Đang khôi phục…" : "Khôi phục"}</button></div>)}</div> : null}</div> : null}
    {newDeviceNotice && draftQuery.data?.current?.deviceLabel ? <div className="mb-3 rounded-2xl border-2 border-sky-300 bg-sky-50 p-4 text-sky-950 shadow-sm" role="alert" aria-live="assertive"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-black">Phát hiện thiết bị mới</p><p className="mt-1 text-xs leading-5">Bạn đang mở tài khoản trên <strong>{deviceLabel}</strong>. Nháp gần nhất được lưu từ <strong>{draftQuery.data.current.deviceLabel}</strong>. Hãy chọn dùng nháp tài khoản hoặc hợp nhất trước khi tiếp tục chỉnh sửa.</p></div><button type="button" onClick={dismissNewDeviceNotice} className="rounded-lg border border-sky-300 bg-white px-3 py-1.5 text-xs font-black text-sky-800">Đã xem</button></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={openMergePanel} disabled={!mergeDiffs.length || saveDraftMutation.isPending} className="rounded-lg bg-sky-700 px-3 py-2 text-xs font-black text-white disabled:opacity-50">Hợp nhất nháp</button><button type="button" onClick={() => { try { const cloud = parseCompanionDraft(draftQuery.data?.current?.data); if (cloud?.companionEmotionMedia) onProfile({ ...profile, companionEmotionMedia: cloud.companionEmotionMedia }, "Đã dùng nháp tài khoản trên thiết bị mới."); } catch { setSyncStatus("error"); } dismissNewDeviceNotice(); }} className="rounded-lg border border-sky-300 bg-white px-3 py-2 text-xs font-black text-sky-800">Dùng nháp tài khoản</button></div></div> : null}
    {showMergePanel && mergeDiffs.length ? <div className="mb-3 rounded-2xl border-2 border-indigo-300 bg-indigo-50 p-4 text-indigo-950" role="dialog" aria-label="Hợp nhất nháp cục bộ và nháp tài khoản"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-black">Hợp nhất dữ liệu nháp</p><p className="mt-1 text-xs leading-5">Chọn cách xử lý từng cảm xúc. “Hợp nhất” giữ cả hai bên; nếu trùng bản thu, bản cục bộ mới hơn sẽ được ưu tiên.</p></div><button type="button" onClick={() => setShowMergePanel(false)} className="rounded-lg border border-indigo-300 bg-white px-3 py-1.5 text-xs font-black text-indigo-800">Đóng</button></div><div className="mt-3 grid gap-2">{mergeDiffs.map((diff) => { const label = emotionThemes.find((theme) => theme.id === diff.emotion)?.label ?? diff.emotion; const choice = mergeChoices[diff.emotion] ?? "merge"; return <div key={diff.emotion} className="rounded-xl border border-indigo-200 bg-white p-3"><div className="flex flex-wrap items-center justify-between gap-2"><span className="text-xs font-black">{label}</span><span className="text-[11px] text-indigo-700">Ảnh local: {diff.local?.lumiImageUrl ? "có" : "chưa có"} · cloud: {diff.cloud?.lumiImageUrl ? "có" : "chưa có"}</span></div><div className="mt-2 flex flex-wrap gap-2">{(["merge", "local", "cloud"] as DraftSource[]).map((source) => <button key={source} type="button" onClick={() => setMergeChoices((current) => ({ ...current, [diff.emotion]: source }))} className={`rounded-lg border px-2.5 py-1.5 text-[11px] font-black ${choice === source ? "border-indigo-700 bg-indigo-700 text-white" : "border-indigo-200 bg-white text-indigo-800"}`}>{source === "merge" ? "Hợp nhất cả hai" : source === "local" ? "Giữ local" : "Giữ cloud"}</button>)}</div></div>; })}</div><button type="button" onClick={applyMerge} disabled={saveDraftMutation.isPending} className="mt-3 w-full rounded-xl bg-indigo-700 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">{saveDraftMutation.isPending ? "Đang hợp nhất và lưu…" : "Xác nhận hợp nhất thành phiên bản mới"}</button></div> : null}
