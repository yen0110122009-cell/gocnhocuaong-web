import { ArchiveRestore, Download, FileUp, Music2, Pause, PenLine, Play, Plus, SlidersHorizontal, Sparkles, Star, Trash2, Upload } from "lucide-react";
...
export function PersonalStudySpaceControls({ profile, emotion, onEmotion, onProfile }: Props) {
  ...
  function exportPersonalSpace() {
    const payload = { version: 1, exportedAt: new Date().toISOString(), personalAudioAssets: assets, personalAudioTrash: trashedAssets, personalStudyPresets: presets };
  ...
  function importPersonalSpace(file: File) {
    ...
    reader.onload = () => {
      try {
        const parsed = JSON.parse(String(reader.result)) as { version?: unknown; personalAudioAssets?: unknown; personalAudioTrash?: unknown; personalStudyPresets?: unknown };
        if (parsed.version !== 1 || !Array.isArray(parsed.personalAudioAssets) || !Array.isArray(parsed.personalStudyPresets)) throw new Error();
        ...
        onProfile({ ...profile, personalAudioAssets: keep(assets, importAssets), personalAudioTrash: keep(trashedAssets, importTrash), personalStudyPresets: keep(presets, importPresets) }, `Đã nhập ${importAssets.length} âm thanh và ${importPresets.length} preset. Liên kết ngoài chỉ hoạt động khi còn hợp lệ.`);
  ...
  function addAsset(nextUrl: string, source: PersonalAudioAsset["source"], fileName?: string) {
    ...
    saveAssets([...assets, { id: createId("personal-audio"), name: name.trim().slice(0, 100), description: fileName, url: safeUrl, source, category, target: normalizedTarget, enabled: true, isDefault: !hasDefault, volume: 70, createdAt: now, updatedAt: now }], "Đã thêm âm thanh vào thư viện cá nhân.");
  ...
  function trashAsset(asset: PersonalAudioAsset) {
    ...
    saveAudioLibrary(assets.filter((item) => item.id !== asset.id), [{ ...asset, deletedAt, updatedAt: deletedAt }, ...trashedAssets].slice(0, 300), `Đã chuyển “${asset.name}” vào Thùng rác âm thanh.`);
  }
  function restoreAsset(asset: PersonalAudioAsset) {
    ...
    saveAudioLibrary([...assets, { ...restored, isDefault: restored.isDefault && !sameDefault, updatedAt: new Date().toISOString() }], trashedAssets.filter((item) => item.id !== asset.id), `Đã khôi phục “${asset.name}” vào thư viện.`);
  }
  function permanentlyDeleteAsset(asset: PersonalAudioAsset) {
    ...
    saveAudioLibrary(assets, trashedAssets.filter((item) => item.id !== asset.id), `Đã xóa vĩnh viễn “${asset.name}”.`);
  }
  ...
  return <PersistentCollapsible storageKey="personal-study-space" eyebrow="Cá nhân hóa" title="Âm thanh & Chủ đề của tôi" className="relative z-10 mt-4 border-[#2e7d32]/20 bg-white/85">
    <section className="space-y-4" aria-label="Không gian học cá nhân">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-black text-[#7f1d1d]">Không gian học cá nhân</p><p className="mt-1 max-w-2xl text-xs leading-5 text-[#35523a]">Tạo bộ màu, cảnh, âm thanh và đồng hành riêng. Hệ thống chỉ phát tệp khi Ong chủ động nhấn nút nghe hoặc bắt đầu phiên học.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={exportPersonalSpace} className="rounded-xl border border-[#2e7d32]/25 bg-white px-3 py-2 text-xs font-black text-[#2e7d32]"><Download className="mr-1 inline h-3.5 w-3.5" />Sao lưu</button><input ref={backupInputRef} type="file" accept="application/json,.json" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) importPersonalSpace(file); event.target.value = ""; }} /><button type="button" onClick={() => backupInputRef.current?.click()} className="rounded-xl border border-[#2e7d32]/25 bg-white px-3 py-2 text-xs font-black text-[#2e7d32]"><FileUp className="mr-1 inline h-3.5 w-3.5" />Nhập</button><button type="button" onClick={randomMix} className="rounded-xl bg-[#c62828] px-3 py-2 text-xs font-black text-white"><Sparkles className="mr-1 inline h-3.5 w-3.5" />Mix ngẫu nhiên</button></div></div>
