...
export function PersonalStudySpaceControls({ profile, emotion, onEmotion, onProfile }: Props) {
  ...
  const [backupName, setBackupName] = useState("Sao lưu Không gian học");
  ...
  function exportPersonalSpace() {
    ...
    const safeName = backupName.trim().replace(/[\\/:*?"<>|]+/g, "-").replace(/\s+/g, " ").slice(0, 80) || "Sao lưu Không gian học";
    const tags = Array.from(new Set(backupTags.split(",").map((tag) => tag.trim().slice(0, 30)).filter(Boolean))).slice(0, 20);
    const payload = {
      version: 3,
      backup: { name: safeName, tags, exportedAt },
      exportedAt,
    ...
    const filename = `${safeName.toLocaleLowerCase().replace(/[^a-z0-9À-ỹ]+/gi, "-").replace(/^-+|-+$/g, "") || "sao-luu-khong-gian-hoc"}-${exportedAt.slice(0, 10)}.json`;
    const link = document.createElement("a");     link.href = URL.createObjectURL(new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" })); link.download = filename; link.click(); URL.revokeObjectURL(link.href);

    onProfile(profile, `Đã xuất “${safeName}”${tags.length ? ` với ${tags.length} tag.` : "."} Tệp chỉ lưu liên kết audio, không sao chép tệp âm thanh.`);
  }
  ...
  return <PersistentCollapsible storageKey="personal-study-space" eyebrow="Cá nhân hóa" title="Âm thanh & Chủ đề của tôi" className="relative z-10 mt-4 border-[#2e7d32]/20 bg-white/85">
    <section className="space-y-4" aria-label="Không gian học cá nhân">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-black text-[#7f1d1d]">Không gian học cá nhân</p><p className="mt-1 max-w-2xl text-xs leading-5 text-[#35523a]">Tạo bộ màu, cảnh, âm thanh và đồng hành riêng. Hệ thống chỉ phát tệp khi Ong chủ động nhấn nút nghe hoặc bắt đầu phiên học.</p></div><div className="flex w-full flex-wrap items-end gap-2 lg:w-auto"><label className="min-w-52 flex-1 text-[11px] font-bold text-[#35523a]">Tên file sao lưu<input value={backupName} onChange={(event) => setBackupName(event.target.value.slice(0, 80))} className="mt-1 w-full rounded-xl border border-[#2e7d32]/25 bg-white px-3 py-2 text-xs" placeholder="Ví dụ: Không gian học tháng 8" aria-label="Tên file sao lưu" /></label><label className="min-w-52 flex-1 text-[11px] font-bold text-[#35523a]">Tag, cách nhau bằng dấu phẩy<input value={backupTags} onChange={(event) => setBackupTags(event.target.value.slice(0, 400))} className="mt-1 w-full rounded-xl border border-[#2e7d32]/25 bg-white px-3 py-2 text-xs" placeholder="tháng 8, trước kỳ thi, an toàn" aria-label="Tag file sao lưu" /></label><button type="button" onClick={exportPersonalSpace} className="rounded-xl border border-[#2e7d32]/25 bg-white px-3 py-2 text-xs font-black text-[#2e7d32]"><Download className="mr-1 inline h-3.5 w-3.5" />Sao lưu preset + nhật ký</button><input ref={backupInputRef} type="file" accept="application/json,.json" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) importPersonalSpace(file); event.target.value = ""; }} /><select value={importMode} onChange={(event) => setImportMode(event.target.value as typeof importMode)} className="rounded-xl border border-[#2e7d32]/25 bg-white px-2 py-2 text-xs font-bold text-[#2e7d32]" aria-label="Cách nhập sao lưu"><option value="merge">Nhập và hợp nhất</option><option value="replace">Nhập và thay thế</option></select><button type="button" onClick={() => backupInputRef.current?.click()} className="rounded-xl border border-[#2e7d32]/25 bg-white px-3 py-2 text-xs font-black text-[#2e7d32]"><FileUp className="mr-1 inline h-3.5 w-3.5" />Nhập sao lưu</button><button type="button" onClick={randomMix} className="rounded-xl bg-[#c62828] px-3 py-2 text-xs font-black text-white"><Sparkles className="mr-1 inline h-3.5 w-3.5" />Mix ngẫu nhiên</button></div></div>
