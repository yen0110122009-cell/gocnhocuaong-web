...
export function PersonalStudySpaceControls({ profile, emotion, onEmotion, onProfile }: Props) {
  ...
  return <PersistentCollapsible storageKey="personal-study-space" eyebrow="Cá nhân hóa" title="Âm thanh & Chủ đề của tôi" className="relative z-10 mt-4 border-[#2e7d32]/20 bg-white/85">
    <section className="space-y-4" aria-label="Không gian học cá nhân">
      ...
      <PersistentCollapsible storageKey="personal-study-schedule" eyebrow="Tự động hóa" title="Lịch preset theo tuần" className="border-[#2e7d32]/20 bg-white/90"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{[1, 2, 3, 4, 5, 6, 0].map((day) => <label key={day} className="text-xs font-bold text-[#35523a]">{["Chủ nhật", "Thứ hai", "Thứ ba", "Thứ tư", "Thứ năm", "Thứ sáu", "Thứ bảy"][day]}<select value={schedule.find((item) => item.dayOfWeek === day)?.presetId ?? "none"} onChange={(event) => updatePresetSchedule(day, event.target.value)} className="mt-1 w-full rounded-lg border border-[#2e7d32]/20 bg-white p-2"><option value="none">Không tự đổi</option>{presets.map((preset) => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</select></label>)}</div><p className="mt-3 text-[11px] text-[#6f5a53]"><CalendarDays className="mr-1 inline h-3.5 w-3.5" />Lịch được lưu riêng cho từng ngày trong tuần.</p></PersistentCollapsible>
      <PersistentCollapsible storageKey="personal-study-history" eyebrow="Khôi phục" title={`Lịch sử thay đổi preset (${presetHistory.length})`} className="border-[#2e7d32]/20 bg-[#fffaf0]"><div className="space-y-2">{presetHistory.length ? presetHistory.slice(0, 12).map((entry) => <div key={entry.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#2e7d32]/15 bg-white p-3"><div><p className="text-sm font-black text-[#35523a]">{entry.presetName}</p><p className="text-xs text-[#6f5a53]">{new Date(entry.changedAt).toLocaleString("vi-VN")} · {entry.reason ?? "Thay đổi preset"}</p></div><button type="button" onClick={() => restorePresetHistory(entry)} className="rounded-lg bg-[#eff9ef] px-2.5 py-1.5 text-xs font-black text-[#2e7d32]"><History className="mr-1 inline h-3.5 w-3.5" />Khôi phục</button></div>) : <p className="text-sm text-[#6f5a53]">Chưa có thay đổi preset để khôi phục.</p>}</div></PersistentCollapsible>
    </section>
