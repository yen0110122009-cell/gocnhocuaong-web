...
type Props = { className?: string; imageUrl?: string; size?: "sm" | "md" | "lg"; label?: boolean };

export function OngLearnerAvatar({ className = "", imageUrl = DEFAULT_IMAGE, size = "md", label = false }: Props) {
  ...
  return <span className={`ong-learner-avatar inline-flex flex-col items-center gap-1 ${className}`}>
    <span className={`${sizeClass} relative overflow-hidden rounded-2xl border-2 border-[#f4b942] bg-gradient-to-br from-[#fff7c7] via-[#ffd95a] to-[#f4b942] shadow-md`}>
      {!failed ? <img src={imageUrl} alt="Ong vàng, hình đại diện của người học" className="h-full w-full object-cover object-top" onError={() => setFailed(true)} /> : <svg viewBox="0 0 120 140" role="img" aria-label="Ong vàng, hình đại diện của người học" className="h-full w-full p-2">
    ...
    {label ? <span className="text-[10px] font-black uppercase tracking-wide text-[#7f1d1d]">Ong vàng · người học</span> : null}
  </span>;
}
