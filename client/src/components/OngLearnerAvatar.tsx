...
export function OngLearnerAvatar({ className = "", imageUrl, variant = "human", size = "md", label = false }: Props) {
  const selectedImage = imageUrl ?? ONG_MASCOT_ASSETS[variant];
  const mascotAlt = variant === "hoodie" ? "Ong mặc hoodie, biến thể người học" : variant === "chibi" ? "Ong chibi, biến thể mascot" : "Ong phong cách người, avatar của người học";
  ...
  return <span className={`ong-learner-avatar inline-flex flex-col items-center gap-1 ${className}`}>
    ...
    {label ? <span className="text-[10px] font-black uppercase tracking-wide text-[#7f1d1d]">Ong · người học</span> : null}
  </span>;
}
