...
export function ExperienceStudio({ selected, onSelect, profile, onProfile, onStartTwoMinutes, customContent = [], mascotStates = [], voiceLines = [] }: Props) {
  ...
  const favoriteAmbientScenes = profile?.favoriteAmbientScenes ?? [];
  ...
  return <PersistentCollapsible storageKey="experience-lumi-emotion-space" eyebrow="Không gian cá nhân" title="Không gian cảm xúc của Lumi" defaultOpen className="relative z-10 border-2 border-[#c62828]/15 bg-[linear-gradient(135deg,#fff7f2_0%,#f5fff5_100%)]"><section className="emotion-studio relative overflow-hidden" aria-labelledby="emotion-studio-title">
    ...
    {profile && onProfile ? <PersistentCollapsible storageKey="experience-lumi-favorite-scenes" eyebrow="Giao diện của Lumi" title="Giao diện yêu thích" className="relative z-10 mt-4 border-amber-200 bg-amber-50/80">{favoriteAmbientScenes.length === 0 ? <p className="text-xs font-medium text-amber-900/75">Chưa có giao diện yêu thích. Khu này sẽ tự hiện khi Ong lưu một giao diện.</p> : <><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-amber-800">Giao diện yêu thích</p><p className="mt-1 text-xs text-amber-900/75">Nhấn một giao diện để áp dụng hoặc bấm sao để bỏ khỏi danh sách.</p></div><Star className="h-5 w-5 fill-amber-400 text-amber-500" aria-hidden="true" /></div><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">{sceneOptions.filter((scene) => favoriteAmbientScenes.includes(scene.id)).map((scene) => <div key={scene.id} className={`rounded-xl border p-2 transition ${ambientScene === scene.id ? "border-amber-500 bg-white shadow-sm" : "border-amber-200 bg-white/70"}`}><div className="flex items-center justify-between gap-1"><button type="button" aria-pressed={ambientScene === scene.id} onClick={() => setScene(scene.id)} className="min-w-0 flex-1 truncate text-left text-xs font-black text-amber-950">{scene.label}</button><button type="button" aria-label={`Bỏ yêu thích ${scene.label}`} onClick={() => toggleFavoriteAmbientScene(scene.id)} className="rounded-md p-1 hover:bg-amber-100"><Star className="h-3.5 w-3.5 fill-amber-400 text-amber-500" /></button></div><p className="mt-1 text-[10px] text-amber-900/70">Đã yêu thích</p></div>)}</div></>}</PersistentCollapsible> : null}
