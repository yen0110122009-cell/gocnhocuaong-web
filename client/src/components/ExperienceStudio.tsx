...
export function ExperienceStudio({ selected, onSelect, profile, onProfile, onStartTwoMinutes, customContent = [], mascotStates = [], voiceLines = [] }: Props) {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (voice) setPlaybackStatus((current) => ({ ...current, voice: { ...current.voice, currentTime: voice.currentTime, duration: Number.isFinite(voice.duration) ? voice.duration : current.voice.duration } }));
      if (environment) setPlaybackStatus((current) => ({ ...current, environment: { ...current.environment, currentTime: environment.currentTime, duration: Number.isFinite(environment.duration) ? environment.duration : current.environment.duration } }));
  ...
  function setChannelPlaying(channel: PlaybackChannel, active: boolean, label = "", url?: string) { setPlaybackStatus((current) => ({ ...current, [channel]: { active, label, url: active ? url : undefined, currentTime: active ? current[channel].currentTime ?? 0 : 0, duration: active ? current[channel].duration : undefined } })); }
