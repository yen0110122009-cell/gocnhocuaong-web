...
type Props = { selected: EmotionId; onSelect: (id: EmotionId) => void; onStartTwoMinutes?: () => void; customContent?: AppConfig["customContent"]; attentionPreferences?: AttentionPreferences; onAttentionPreference?: (key: keyof AttentionPreferences, value: boolean) => void };
...
export function ExperienceStudio({ selected, onSelect, onStartTwoMinutes, customContent = [] }: Props) {
  ...
  const [attentionPreferences, setAttentionPreferences] = useState<AttentionPreferences>(() => {
    try { const saved = window.localStorage.getItem("study-empire:attention-preferences"); const parsed = saved ? JSON.parse(saved) as Partial<AttentionPreferences> : {}; return { animationsEnabled: parsed.animationsEnabled !== false, popupsEnabled: parsed.popupsEnabled !== false, soundEnabled: parsed.soundEnabled !== false }; }
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent("study-empire:attention-preferences", { detail: attentionPreferences }));
  }, [attentionPreferences]);

  function toggleAttentionPreference(key: keyof AttentionPreferences) {
    setAttentionPreferences((current) => ({ ...current, [key]: !current[key] }));
  }
  ...
  return <section className="panel emotion-studio border-2 border-[#c62828]/15 bg-[linear-gradient(135deg,#fff7f2_0%,#f5fff5_100%)] p-5 sm:p-6" aria-labelledby="emotion-studio-title">
    ...
    <AttentionControls preferences={attentionPreferences} onToggle={toggleAttentionPreference} />
...
function AttentionControls({ preferences, onToggle }: { preferences: AttentionPreferences; onToggle: (key: keyof AttentionPreferences) => void }) {
  const items: Array<{ key: keyof AttentionPreferences; label: string; detail: string }> = [
    { key: "animationsEnabled", label: "Hoạt ảnh", detail: "Hiệu ứng chuyển động và ăn mừng" },
    { key: "popupsEnabled", label: "Popup", detail: "Thông báo nổi không thiết yếu" },
    { key: "soundEnabled", label: "Âm thanh", detail: "Âm nền và âm báo sau thao tác" },
  ];
  return <section className="mt-5 rounded-2xl border border-[#2e7d32]/20 bg-white/75 p-4" aria-label="Điều khiển giảm mất tập trung"><div><p className="text-xs font-black uppercase tracking-wider text-[#2e7d32]">Tập trung theo cách của Ong</p><p className="mt-1 text-xs leading-5 text-[#35523a]">Mỗi mục được bật/tắt riêng. Các lựa chọn được lưu trên thiết bị này.</p></div><div className="mt-3 grid gap-2 sm:grid-cols-3">{items.map((item) => <button key={item.key} type="button" aria-pressed={preferences[item.key]} onClick={() => onToggle(item.key)} className={`rounded-xl border px-3 py-3 text-left transition ${preferences[item.key] ? "border-[#2e7d32] bg-[#eff9ef] text-[#245c2b]" : "border-slate-200 bg-slate-50 text-slate-600"}`}><span className="flex items-center justify-between gap-2"><b className="text-sm">{item.label}</b><span className="text-xs font-black">{preferences[item.key] ? "Đang bật" : "Đang tắt"}</span></span><small className="mt-1 block leading-4 opacity-80">{item.detail}</small></button>)}</div></section>;
}
