...
export function ExperienceStudio({ selected, onSelect, profile, onProfile, onStartTwoMinutes, customContent = [], mascotStates = [], voiceLines = [] }: Props) {
  ...
  return <PersistentCollapsible storageKey="experience-lumi-emotion-space" eyebrow="Không gian cá nhân" title="Không gian cảm xúc của Lumi" className="relative z-10 border-2 border-[#c62828]/15 bg-[linear-gradient(135deg,#fff7f2_0%,#f5fff5_100%)]"><section className="emotion-studio relative overflow-hidden" aria-labelledby="emotion-studio-title">
    ...
    <section className="rounded-2xl" aria-label="Âm thanh và cảnh nền">
      <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-black uppercase tracking-wider text-[#2e7d32]">Âm thanh và cảnh nền</p><p className="mt-1 text-xs text-[#35523a]">Cảnh nền sẽ tự phát nếu trình duyệt cho phép. Nếu autoplay bị chặn, hãy nhấn nút <b>Phát thử âm nền</b> để chủ động kích hoạt.</p></div><div className="flex flex-wrap items-center gap-2">{!audioUnlocked ? <button type="button" onClick={() => { void unlockAudio(); }} className="rounded-xl border-2 border-[#2e7d32]/30 bg-[#eff9ef] px-3 py-2 text-xs font-black text-[#2e7d32] shadow-sm">Mở khóa âm thanh trên thiết bị</button> : null}<button type="button" aria-pressed={ambientMuted} onClick={toggleAmbientMute} className="rounded-xl border border-[#c62828]/25 bg-white px-3 py-2 text-xs font-black text-[#c62828]">{ambientMuted ? <><VolumeX className="mr-1 inline h-3.5 w-3.5" />Bật tiếng âm nền</> : <><Volume2 className="mr-1 inline h-3.5 w-3.5" />Mute âm nền</>}</button><button type="button" onClick={() => { void unlockAudio().then(() => toggleAmbient()); }} className="rounded-xl bg-[#c62828] px-3 py-2 text-xs font-black text-white shadow-sm ring-2 ring-[#c62828]/20 ring-offset-1">{ambientPlaying ? <><Pause className="mr-1 inline h-3.5 w-3.5" />Dừng âm nền</> : <><Play className="mr-1 inline h-3.5 w-3.5" />Phát thử âm nền</>}</button></div></div>
