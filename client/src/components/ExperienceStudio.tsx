...
const mascotImages = {
  lumi: "/manus-storage/lumi-mascot-clean_28a6da68.png",
  ong: "/manus-storage/ong-mascot-clean_079128db.png",
} as const;
...
export function ExperienceStudio({ selected, onSelect, onStartTwoMinutes }: Props) {
  ...
  const mascotName = theme.mascot === "lumi" ? "Lumi" : "Ong";
  const mascotAlt = theme.mascot === "lumi" ? "Lumi đeo kính với kẹp tóc hình ngôi sao vàng" : "Ong với kẹp tóc hình ngọn lửa và áo đỏ";
  ...
  return <section className="panel emotion-studio border-2 border-[#c62828]/15 bg-[linear-gradient(135deg,#fff7f2_0%,#f5fff5_100%)] p-5 sm:p-6" aria-labelledby="emotion-studio-title">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div><p className="text-xs font-black uppercase tracking-[.18em] text-[#c62828]">Emotion Theme Studio</p><h2 id="emotion-studio-title" className="mt-2 font-display text-2xl font-black text-[#7f1d1d]">Hôm nay Ong đang cảm thấy thế nào?</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#3f513f]">Chọn một nhịp phù hợp. Giao diện, mascot và lời nhắn sẽ đổi nhẹ theo cảm xúc, không tạo áp lực.</p></div>
      <div className="flex items-center gap-3 rounded-2xl border border-[#2e7d32]/20 bg-white/80 px-3 py-3 shadow-sm"><img src={mascotImages[theme.mascot]} alt={mascotAlt} className="h-20 w-16 rounded-2xl object-cover object-top" /><div><p className="text-xs font-black uppercase tracking-wider text-[#2e7d32]">Mascot đồng hành</p><p className="mt-1 text-sm font-black text-[#7f1d1d]">{mascotName}</p><span className="text-xs text-[#35523a]">{theme.label}</span></div></div>
    </div>
