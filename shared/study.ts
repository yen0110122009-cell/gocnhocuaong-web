...
export type AmbientScenePreference = "morning" | "rain" | "snow" | "leaves" | "storm" | "summer" | "spring" | "tet" | "halloween" | "desert" | "night" | "naturepark" | "sunrise" | "mountainsunset" | "meteorice" | "galaxy" | "cityday" | "citysunset" | "citydusk" | "citynight" | "bridgefog" | "urbanfog" | "sparklers" | "fireworks" | "forest" | "sunset" | "space" | "crescentmoon" | "ocean" | "neon" | "sakura" | "autumn" | "festival" | "volcano" | "deepocean" | "magicforest" | "spacestation" | "flowerfield" | "fairytale" | "circus" | "prehistoric" | "cyberrace" | "foodfestival" | "diamondmine" | "f1race" | "candykingdom" | "travel" | "tropical" | "sweet_strawberry" | "black_ribbon" | "library_chill" | "after_school" | "classic_academy" | "cyber_highschool" | "rainy_season" | "stormy_season" | "morning_chill" | "pixel" | "pirate" | "sports" | "disco" | "laboratory" | "egypt" | "steampunk" | "art" | "ninja" | "coffee" | "ai" | "teddy" | "spring-blossom" | "summer-beach" | "autumn-leave" | "winter-snow" | "halloween-spooky" | "lunar-new-year" | "thunder-storm" | "rainy-day" | "sunny-day" | "foggy-morning";
...
export function normalizeProfile(value: unknown): ProfileState {
  ...
  const merged: ProfileState = {
    ...
    defaultAmbientScene: ["morning", "rain", "snow", "leaves", "storm", "summer", "spring", "tet", "halloween", "desert", "night", "naturepark", "sunrise", "mountainsunset", "meteorice", "galaxy", "cityday", "citysunset", "citydusk", "citynight", "bridgefog", "urbanfog", "sparklers", "fireworks", "forest", "sunset", "space", "crescentmoon", "ocean", "neon", "sakura", "autumn", "festival", "volcano", "deepocean", "magicforest", "spacestation", "flowerfield", "fairytale", "circus", "prehistoric", "cyberrace", "foodfestival", "diamondmine", "f1race", "candykingdom", "travel", "tropical"].includes(String(source.defaultAmbientScene)) ? source.defaultAmbientScene as AmbientScenePreference : base.defaultAmbientScene,
    ...
    sceneAutomation: {
      ...
      timeRules: Array.isArray(source.sceneAutomation?.timeRules) ? source.sceneAutomation.timeRules.flatMap((rule, index) => {
        ...
        if (!candidate.id || !["morning", "rain", "snow", "leaves", "storm", "summer", "spring", "tet", "halloween", "desert", "night", "naturepark", "sunrise", "mountainsunset", "meteorice", "galaxy", "cityday", "citysunset", "citydusk", "citynight", "bridgefog", "urbanfog", "sparklers", "fireworks", "forest", "sunset", "space", "crescentmoon", "ocean", "neon", "sakura", "autumn", "festival", "volcano", "deepocean", "magicforest", "spacestation", "flowerfield", "fairytale", "circus", "prehistoric", "cyberrace", "foodfestival", "diamondmine", "f1race", "candykingdom", "travel", "tropical", "sweet_strawberry", "black_ribbon", "library_chill", "after_school", "classic_academy", "cyber_highschool"].includes(String(candidate.scene))) return [];
    ...
    audioMixer: {
      ambientSceneVolumes: Object.fromEntries((["morning", "rain", "snow", "leaves", "storm", "summer", "spring", "tet", "halloween", "desert", "night", "naturepark", "sunrise", "mountainsunset", "meteorice", "galaxy", "cityday", "citysunset", "citydusk", "citynight", "bridgefog", "urbanfog", "sparklers", "fireworks", "forest", "sunset", "space", "crescentmoon", "ocean", "neon", "sakura", "autumn", "festival", "volcano", "deepocean", "magicforest", "spacestation", "flowerfield", "fairytale", "circus", "prehistoric", "cyberrace", "foodfestival", "diamondmine", "f1race", "candykingdom", "travel", "tropical", "sweet_strawberry", "black_ribbon", "library_chill", "after_school", "classic_academy", "cyber_highschool"] as AmbientScenePreference[]).map((scene) => [scene, Math.max(0, Math.min(100, Number(source.audioMixer?.ambientSceneVolumes?.[scene] ?? base.audioMixer!.ambientSceneVolumes[scene]) || 0))])) as AudioMixerSettings["ambientSceneVolumes"],
    ...
    personalStudyPresets: Array.isArray(source.personalStudyPresets) ? source.personalStudyPresets.flatMap((value) => {
      ...
      const name = typeof preset?.name === "string" && preset.name.trim() ? preset.name.trim().slice(0, 80) : "";
      if (!id || !name) return [];
      return [{ id, name, emotion: ["calm", "happy", "tired", "sad", "stressed", "lazy", "proud", "focused", "hopeful", "overwhelmed", "sleepy", "excited", "lonely", "confident", "curious", "comeback"].includes(String(preset?.emotion)) ? preset?.emotion as EmotionThemeId : undefined, ambientScene: ["morning", "rain", "snow", "leaves", "storm", "summer", "spring", "tet", "halloween", "desert", "night", "naturepark", "sunrise", "mountainsunset", "meteorice", "galaxy", "cityday", "citysunset", "citydusk", "citynight", "bridgefog", "urbanfog", "sparklers", "fireworks", "forest", "sunset", "space", "crescentmoon", "ocean", "neon", "sakura", "autumn", "festival", "volcano", "deepocean", "magicforest", "spacestation", "flowerfield", "fairytale", "circus", "prehistoric", "cyberrace", "foodfestival", "diamondmine", "f1race", "candykingdom", "travel", "tropical"].includes(String(preset?.ambientScene)) ? preset?.ambientScene as AmbientScenePreference : undefined, audioAssetIds: Array.isArray(preset?.audioAssetIds) ? preset!.audioAssetIds.filter((id): id is string => typeof id === "string" && id.trim().length > 0).slice(0, 80) : [], companionMode: ["lumi", "ong", "both", "hidden"].includes(String(preset?.companionMode)) ? preset?.companionMode as PersonalStudyPreset["companionMode"] : "both", focusMode: preset?.focusMode === true, createdAt: typeof preset?.createdAt === "string" && preset.createdAt ? preset.createdAt : new Date(0).toISOString(), updatedAt: typeof preset?.updatedAt === "string" && preset.updatedAt ? preset.updatedAt : new Date(0).toISOString() }];
    }).filter((preset, index, presets) => presets.findIndex((candidate) => candidate.id === preset.id) === index).slice(0, 100) : [],
